...
Motivation: 由于VLM研究的多样性（如开发新模型、领域微调或直接应用），传统机器学习报告标准难以满足需求，亟需新的标准化协议。

Method: 提出了一种VLM研究的分类框架，并制定了相应的报告标准，涵盖性能评估、数据报告和论文撰写建议。

Result: 设计了一个标准化检查清单，以确保VLM相关研究的一致性和质量。

Conclusion: 通过分类框架和报告标准，本文为VLM研究提供了更直观且严谨的指导，促进社区采纳。
...
<details>
  <summary>Details</summary>
Motivation: 交通网络安全生态系统的快速发展需要理解其时空动态和影响因素，以支持经济和劳动力规划。

Method: 开发BiTransGCN框架，结合Transformer和GCN，分析访客流模式及其社会经济驱动因素。

Result: 框架能有效预测行业集群和劳动力分布变化，为战略规划提供数据支持。

Conclusion: 研究为交通网络安全生态系统的经济规划和投资提供了数据驱动的基础。
...
<details>
  <summary>Details</summary>
Motivation: 了解人工智能和机器人技术对全球劳动力市场的影响，特别是在新兴经济体中公众的感知变化。

Method: 利用2017、2018、2020和2023年的Latinobarómetro调查数据，对48,000多名受访者进行统计建模和潜在类别分析。

Result: 发现教育和政治倾向是最一致的担忧驱动因素，2018年担忧达到峰值，且存在跨国差异。

Conclusion: 研究揭示了新兴经济体中AI焦虑的社会和结构维度，补充了全球北方以外的自动化态度研究。
...
<details>
  <summary>Details</summary>
Motivation: 探讨算法封禁对零工经济工人的财务影响，并解决现有政策下缺乏有效工具支持的问题。

Method: 与工会合作设计FareShare工具，自动化工资损失计算，并在实际场景中部署测试。

Result: 工具将工资计算时间减少95%，消除人工错误，并提升法律团队效率。

Conclusion: FareShare工具在解决算法封禁问题上有效，但也揭示了信任和采纳等社会技术挑战。
...
<details>
  <summary>Details</summary>
Motivation: 随着老年护理资源稀缺，社交机器人被提出作为解决方案，但其伦理挑战需系统评估以支持负责任的应用。

Method: 通过电子数据库检索关键词，筛选248篇文献，采用定性内容分析法提取数据。

Result: 识别出60个广泛伦理问题，涉及三类利益相关者，发现许多潜在相关但未被充分讨论的伦理问题。

Conclusion: 决策者需理解伦理讨论的具体性质，以进行细致的伦理考量，实现社交机器人在老年护理中的负责任应用。
...
### [6] [Toward Fair Federated Learning under Demographic Disparities and Data Imbalance](https://arxiv.org/abs/2505.09295)
...
<details>
  <summary>Details</summary>
Motivation: 在医疗等高风险领域，AI模型可能因数据不平衡和人口统计偏差加剧现有不平等，联邦学习虽保护隐私但仍存在偏见问题。

Method: 结合公平感知正则化和组条件过采样，支持多敏感属性和异构数据分布，不影响联邦学习收敛性。

Result: 理论分析证明公平性改进边界，实验显示FedIDA在公平性和预测性能上均表现优异。

Conclusion: FedIDA有效提升公平性并保持预测性能，适用于医疗领域的隐私保护建模。
...
<details>
  <summary>Details</summary>
Motivation: 研究RLHF技术对生成式AI聊天机器人的影响，以及其在语言规范、信息搜索和社会关系方面的伦理和社会技术问题。

Method: 采用修辞分析，特别是Ian Bogost的程序修辞理论，从内容分析转向RLHF增强的LLMs的底层说服机制。

Result: 揭示了RLHF技术可能强化霸权语言使用、延续偏见、去语境化学习并侵蚀人际关系。
...
<details>
  <summary>Details</summary>
Motivation: 随着LLM在各行业的广泛应用，理解其在推理阶段的环境影响变得至关重要，但现有研究多忽略专有模型、基础设施差异或仅关注训练阶段。

Method: 结合公共API性能数据、区域特定环境乘数和硬件配置统计推断，并采用交叉效率DEA方法对模型进行生态效率排名。

Result: 研究发现o3和DeepSeek-R1能耗最高（33 Wh/长提示），而Claude-3.7 Sonnet生态效率最佳；GPT-4o单次短查询能耗0.43 Wh，但全球规模下环境影响巨大。

Conclusion: 研究为LLM部署的可持续性提供了标准化基准方法，强调了全球规模下资源消耗的悖论，为未来AI环境责任和可持续发展标准奠定基础。
...
<details>
  <summary>Details</summary>
Motivation: 随着直播流媒体的普及，需要低延迟的实时内容审核来移除危险或违规内容。

Method: 扩展Media Over QUIC Transport协议，支持实时内容审核，并将分析任务透明分配给客户端设备。

Result: 系统在光敏观众的光闪烁移除场景中测试，客户端延迟仅增加一个GOP（图像组）时长。

Conclusion: 该方法有效实现了直播流媒体的实时内容审核，同时保持了低延迟。
...
### [10] [Multi-source Plume Tracing via Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2505.08825)
...
Method: 采用多智能体强化学习（MARL），将问题建模为部分可观察马尔可夫博弈（POMG），使用基于LSTM的动作特定双深度循环Q网络（ADDRQN），并结合高斯羽流模型（GPM）进行仿真。
...
<details>
  <summary>Details</summary>
Motivation: 传统多智能体路径规划（MAPF）不适用于周期性且长时间运行的流水线场景，因此需要一种新的解决方案。

Method: 提出了ASCBS算法，通过引入周期性顶点/边约束处理冲突，并探索了不相交分割策略的应用。

Result: 实验表明，ASCBS在长时间运行场景中的运行时间优于传统MAPF求解器。

Conclusion: ASCBS为流水线场景提供了一种高效的路径规划解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 信号交叉口的复杂决策过程使得自动驾驶车辆控制具有挑战性，需要一种高效且安全的控制策略。

Method: 采用深度强化学习方法，结合DDPG和SAC算法，设计了一个综合奖励函数，重点关注距离车头效率、黄灯决策和非对称加减速响应。

Result: RL模型在保持较低距离车头和较小急动度的同时，未牺牲安全性，且在安全关键场景中表现稳健。

Conclusion: DRL策略显著提升了信号交叉口的交通效率、安全性和舒适性，DDPG模型在动作平滑性上优于SAC。
...
<details>
  <summary>Details</summary>
Motivation: 人工构建评估基准耗时且难以覆盖所有领域，需要自动化方法。

Method: 利用语言模型和基础文档自动生成合成数据问题，支持多选和开放式问题。

Result: 合成数据评估与人工标注相关性高（Spearman 0.96，Pearson 0.79），Gemma3模型表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 现实场景中奖励往往不可观测，而现有Mon-MDP研究局限于简单表格情况，限制了实际应用。本文旨在探索函数逼近在Mon-MDP中的潜力。

Method: 结合函数逼近和奖励模型，实现从可观测状态到不可观测状态的泛化，并提出基于奖励不确定性的优化方法以缓解过度泛化。

Result: 研究表明，奖励模型结合函数逼近能在形式上无解的环境中实现接近最优的策略，但也揭示了过度泛化导致的不良行为。

Conclusion: 本文为Mon-MDP理论与实际应用之间的差距提供了初步解决方案，但仍需进一步研究以解决过度泛化问题。
...
<details>
  <summary>Details</summary>
Motivation: 探索低成本、安全的模拟环境中的防御场景，解决多智能体系统中复杂动态和状态空间挑战。

Method: 将决策分为高低两层：低层控制个体单位，高层指挥官政策发布宏观命令，利用课程学习和预训练策略。

Result: 实证验证了框架的有效性，分层结构简化了训练过程并提升了性能。

Conclusion: 分层多智能体强化学习框架在模拟空战中表现优越，为复杂任务提供了可行解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 电网中的级联故障可能导致电网崩溃，严重影响社会和经济活动。现有策略多为单级设计，忽视了多级场景的复杂性。

Method: 将多级级联故障问题建模为强化学习任务，开发模拟环境，并使用确定性策略梯度算法训练智能体以实现连续动作。

Result: 在IEEE 14总线和118总线系统上验证了所提方法的有效性。

Conclusion: 基于强化学习的多级级联故障缓解策略能够有效应对复杂场景，为电网稳定性提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是探索LLM代理在战略推理中与人类行为的相似性，以及代理设计复杂度对其表现的影响。

Method: 方法包括设计三种代理（简单游戏理论模型、非结构化LLM代理、LLM与传统代理框架结合），并通过猜测游戏和模糊化场景测试其泛化能力。

Result: 结果表明，人类启发式结构能提升LLM代理的对齐性，但代理复杂度与人类相似性关系非线性，且受限于LLM基础能力。

Conclusion: 结论指出，简单架构增强存在局限性，LLM代理的战略推理能力依赖于其基础能力。
...
Method: 采用代理强化学习技术，通过LLM作为法官（LLMaaJ）指导另一LLM生成内容，并结合人类编辑反馈优化文本。

Result: 实验显示，人类内容审阅者的预期与AI生成内容对齐率达53.8%，平均迭代次数为4.38，内容质量显著提升。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于噪声的探索方法（如TD3）可能导致策略收敛不理想，需要更高效的探索方法。

Method: MCBS通过生成候选动作并评估短视界模拟结果，结合TD3改进动作选择。

Result: 在多个基准测试中，MCBS表现出更高的样本效率和性能，收敛速度更快。
...
<details>
  <summary>Details</summary>
Motivation: LLMs在复杂任务中常生成自信但错误或不相关的信息（幻觉），现有方法如CoT未能完全解决此问题。

Method: 结合CoT与RAG，并应用自一致性和自验证策略，引入外部知识源并让模型自我验证输出。

Result: 比较了基线LLMs与CoT、CoT+RAG、自一致性和自验证方法，结果显示这些方法能有效减少幻觉并保持流畅性和推理深度。

Conclusion: 研究发现结合多种策略（尤其是CoT+RAG）是最稳健的方法，能显著减少幻觉并提高准确性。
...
<details>
  <summary>Details</summary>
Motivation: 传统决策变换器（DT）依赖高质量离线数据，但在现实应用中数据不足或次优行为稀缺时表现受限。CRDT旨在通过反事实推理解决这一问题。

Method: CRDT引入反事实推理，生成并利用反事实经验，增强DT在未知场景中的决策能力，无需修改架构。

Result: 在Atari和D4RL基准测试中，CRDT在数据有限或动态变化场景下优于传统DT，并具备拼接次优轨迹的能力。

Conclusion: 反事实推理能显著提升强化学习代理的性能和泛化能力，CRDT为此提供了有效框架。
...
<details>
  <summary>Details</summary>
Motivation: 验证和扩展Piatti等人的研究，进一步探索LLMs在复杂合作任务中的适应性和通用性。

Method: 通过复制关键实验和引入新设置（如异构多智能体环境、日语指令场景和逆向环境）来评估模型的合作行为。

Result: 大模型（如GPT-4-turbo）在资源分享中表现更优，无需普遍化原则即可合作；高表现模型能影响低表现模型的行为。

Conclusion: GovSim框架适用于新模型和场景，为开发更高效的合作AI系统提供了重要见解。
...
<details>
  <summary>Details</summary>
Motivation: AI系统面临双用途困境，同一请求可能无害或有害，仅基于内容决策会导致误判。

Method: 提出概念性访问控制框架，结合用户凭证验证和风险分类器，使用小规模专家模块实现高效风险检测。

Result: 框架初步实现细粒度治理，合法用户获取专业知识，阻止恶意访问。

Conclusion: 该上下文方法平衡实用性与安全性，为双用途困境提供解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有反事实方法主要针对单步决策，无法直接应用于序列决策任务，因此需要一种适用于MDP的反事实策略方法。

Method: 通过将反事实策略编码为非线优化问题，并扩展以合成多样化的反事实策略。

Result: 在四个真实数据集上验证了方法的实用性，展示了其在复杂序列决策任务中的可行性。

Conclusion: 该方法为序列决策任务中的反事实分析提供了有效工具，具有实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: POMDPs在不确定性下的顺序决策中建模特定环境，但最优策略可能对环境扰动不鲁棒。HM-POMDPs通过捕捉一组不同的环境模型来解决这一问题。

Method: 结合两种正交技术：(1) 形式化验证计算HM-POMDP中最坏情况的POMDP；(2) 次梯度上升优化候选策略以应对最坏情况。

Result: 实验表明，该方法生成的策略更鲁棒，能更好地泛化到未见过的POMDPs，且可扩展到包含超过十万个环境的HM-POMDPs。

Conclusion: 该方法有效解决了HM-POMDPs中的鲁棒策略问题，具有较好的可扩展性和泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 研究LM代理是否具备探索和理解世界因果结构的能力，是否存在系统性偏差。

Method: 使用‘Blicket Test’范式评估LM的因果推理能力，分析模型家族、大小和提示策略的影响。

Result: LM普遍存在析取偏差，任务复杂度增加时表现更差；与人类成人推理模式相似。

Conclusion: 提出测试时采样方法有效减少偏差，推动LM实现更科学的因果推理。
...
<details>
  <summary>Details</summary>
Motivation: 传统系统（如死机开关）对司机警觉性检测有限，铁路安全面临挑战。

Method: 采用定制DGNN，通过消融研究比较骨骼、面部及两者结合的特征配置。

Result: 结合面部和骨骼特征的三分类模型准确率达80.88%，二分类警觉性检测准确率超99%。

Conclusion: 该研究通过视觉技术提升了铁路安全的在线监测能力，并首次引入模拟病理状态数据集。
...
<details>
  <summary>Details</summary>
Motivation: 现有步态识别系统在真实世界的非约束条件下性能下降，主要受户外环境、光照变化等因素影响，缺乏综合解决这些挑战的数据集。

Method: 采用骨骼模型方法，提取关节标志点生成数值数据集，开发OptiGait-LGBM分类模型，并引入RUET-GAIT基准数据集。

Result: 与随机森林和CatBoost等集成方法相比，OptiGait-LGBM在准确性、内存使用和训练时间上表现更优。
...
<details>
  <summary>Details</summary>
Motivation: 稀疏表示在高精地图构建中效率更高，但现有方法因缺乏针对性设计而性能不足。本文旨在通过系统改进稀疏表示技术，弥补其与密集方法的差距并超越。

Method: 提出专用网络架构优化稀疏特征提取，引入稀疏-密集分割辅助任务以利用几何和语义线索，并通过物理先验引导的去噪模块优化预测。

Result: 在nuScenes数据集上达到最先进性能，SparseMeXt-Tiny、Base和Large分别实现55.5%、65.2%和68.9%的mAP，显著提升了稀疏方法的竞争力。

Conclusion: 稀疏方法在高精地图构建中具有巨大潜力，挑战了传统密集表示的依赖，重新定义了效率与性能的权衡。
...
<details>
  <summary>Details</summary>
Motivation: 水下3D重建因光传播、水体介质和物体表面的复杂交互而困难，现有方法建模不准确且计算成本高。

Method: TUGS采用轻量级张量化高阶高斯和基于物理的自适应介质估计模块（AME），模拟光衰减和背散射效应。

Result: TUGS在真实水下数据集上表现优异，渲染速度快、内存占用低，重建质量高。

Conclusion: TUGS适用于内存受限的水下无人机应用，显著提升了水下3D重建的效率和实用性。
...
<details>
  <summary>Details</summary>
Motivation: 随着DNNs的广泛应用，其安全性测试成为研究热点，但目前缺乏对不同覆盖度量之间关系的实证研究。

Method: 选取LeNet、VGG和ResNet等不同架构的DNN模型，以及5至54层的10个模型，通过实验比较不同深度、配置信息与覆盖度量的关系。

Result: 实验揭示了模型深度、配置信息与覆盖度量之间的具体关系和模式，并探讨了修改后的决策/条件覆盖与数据集大小的关联。
...
<details>
  <summary>Details</summary>
Motivation: 尽管SBM在图像合成方面表现卓越，但其可能生成不适宜内容（如暴力或非自愿裸露）。现有方法多依赖模型自身知识或需微调，本文探索利用外部资源确保安全输出。

Method: 采用Conditional Trajectory Correction步骤，利用多模态模型（如CLIP）引导样本远离不安全区域，并支持用户自定义NSFW类别。

Result: 在Stable Diffusion上的实验表明，SFW采样器显著减少不安全内容生成，且对无需修正的样本影响可忽略。
...
Method: 通过扩展Stable Diffusion模型并集成ControlNet，结合OpenStreetMap的土地利用和约束信息，生成条件化的卫星图像。

Result: 模型在三个美国城市的数据上表现出色，生成多样且真实的城市景观，并通过FID和KID评分验证了其高质量和鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 解决人群计数中深度学习模型需要大量标注数据的问题，以及处理遮挡、非均匀密度等复杂场景的挑战。

Method: 1. 自监督训练减少标注需求；2. 多列CNN处理不同尺度特征；3. 结合VGG19和LSTM的时空模型用于异常检测。

Result: 在ShanghaiTech和UCF-QNRF数据集上表现优异，异常检测模型在Hockey Fight和SCVD数据集上优于现有方法。

Conclusion: 提出的方法在人群计数和异常检测中有效，减少了数据依赖并提升了性能。
...
<details>
  <summary>Details</summary>
Motivation: 探索GenAI如何推动自动驾驶技术，尤其是Level 5全自动驾驶的实现，并解决相关工程挑战。

Method: 通过分析生成模型（如VAEs、GANs、Diffusion Models和LLMs）及其在自动驾驶中的应用（如图像生成、决策制定等），分类实际应用场景。

Result: 总结了GenAI在自动驾驶中的潜力，同时指出了泛化能力、安全性、伦理等关键挑战。

Conclusion: GenAI为自动驾驶提供了重要技术路径，但需进一步研究理论保障、社会影响等问题，为研究者和政策制定者提供了前瞻性参考。
...
Method: 系统结合Raspberry Pi、摄像头模块、深度学习模型和云服务，实时检测、分类道路损坏并发送警告信号。

Result: 系统能够实时检测和分类道路损坏，并向相关部门和驾驶员发送警告，以减少潜在事故。

Conclusion: 该解决方案通过主动检测和预警道路损坏，有望显著提升道路安全，减少交通事故。
...
<details>
  <summary>Details</summary>
Motivation: 皮肤癌发病率上升，但公众意识和临床专业知识不足，亟需先进的诊断辅助工具。

Method: 利用ISIC数据库的皮肤镜图像，结合神经模糊和殖民竞争算法开发诊断方法。

Result: 在560张图像的数据集上实现了94%的准确率。

Conclusion: 该方法在皮肤癌早期诊断中具有显著潜力，可为临床提供有力支持。
...
Conclusion: CoCo去噪器解决了传统方法的局限性，为图像恢复提供了新思路。
...
Conclusion: Maya为多语言和文化多样性的视觉语言任务提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 基于信息瓶颈（IB）原则，旨在选择最具信息量的通道以提升特征提取能力。

Method: 设计了DCS-Attention模块，支持固定或可学习骨干网络（DCS-FB和DCS-DNAS），并推导了IB损失的可变分上界以优化训练。

Result: 在多个行人重识别基准测试中，DCS-Attention显著提升了DNN的预测精度。

Conclusion: DCS-Attention通过选择信息量最大的通道，有效提升了行人重识别任务的性能。
...
<details>
  <summary>Details</summary>
Motivation: 标准LVLMs预训练中，NTP会拟合噪声并增加幻觉风险，因为只有少量标记与视觉内容直接相关。

Method: PRIOR利用文本参考LLM对标记进行重要性评分，调整NTP损失权重，优先处理图像相关标记。

Result: 在两种LVLMs设置中，PRIOR分别实现了19%和8%的平均相对改进，并展示了更好的扩展性。

Conclusion: PRIOR通过差异加权显著提升了性能，并具有更强的扩展潜力。
...
Method: 结合多模型集成和元学习策略，利用动量机制和高斯平滑，随机选择模型并构建多样场景进行白盒和黑盒攻击。
...
<details>
  <summary>Details</summary>
Motivation: 面部识别在姿态差异大的情况下性能下降，需要一种方法提升姿态不变性。

Method: 使用共享注意力映射和联合熵正则化损失，增强2D和3D表示之间的相关性。

Result: 在FaceScape和ARL-VTF数据集上，性能显著优于竞争方法，TAR提升至少7.1%和1.57%。

Conclusion: 该框架有效解决了姿态差异导致的性能下降问题，为面部识别提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 现有LKA系统因专有性和数据限制，真实表现未充分研究，OpenLKA旨在填补这一空白。

Method: 通过路测和社区贡献收集多模态数据，包括CAN总线、视频、Openpilot输出及场景标注。

Result: 数据集提供全面平台，用于评估LKA性能、识别关键场景及道路基础设施适应性。

Conclusion: OpenLKA为LKA系统改进和自动驾驶发展提供了宝贵资源，数据集已公开。
...
<details>
  <summary>Details</summary>
Motivation: 传统场景图主要关注空间关系，限制了视觉语言模型对复杂交互的推理能力。现有方法存在关系集无关和缺乏持久记忆的问题。

Method: ISGR框架包含双流图构建（空间关系提取与交互感知标注）、交互查询激活模型知识、长期记忆强化学习。
...
<details>
  <summary>Details</summary>
Motivation: 尽管之前的研究认为SAM不适用于COD，但本研究证明通过适当的提示，SAM可以胜任此任务，从而避免从头设计专业模型。

Method: 开发了Promotion Point Targeting Network (PPT-net)预测伪装目标的存在概率，并设计Key Point Selection (KPS)算法对比性地应用正负点提示。

Result: 在3个数据集和6个指标上，该方法优于现有方法，展示了SAM在COD任务中的潜力。

Conclusion: 该研究为COD提供了一种现成的方法，不仅性能优越，还将问题简化为寻找信息性但不需精确的提示。
...
<details>
  <summary>Details</summary>
Motivation: 传统深度学习模型在高风险安全任务中面临未标记和数据不可利用的挑战，需要一种轻量级解决方案。

Method: 融合无监督KMeans聚类和RGB通道直方图建模，检测关键帧中的结构异常和颜色突变信号。

Result: 在非洲国家操作监控视频中成功识别高异常帧，适用于战术警告、可疑物体筛选和环境变化监测。
...
### [49] [Beyond General Prompts: Automated Prompt Refinement using Contrastive Class Alignment Scores for Disambiguating Objects in Vision-Language Models](https://arxiv.org/abs/2505.09139)
...
Method: 通过大语言模型生成多样提示候选，并使用CCAS（基于句子变换器的嵌入）筛选最佳提示。

Result: 在挑战性目标类别上验证，自动选择的高精度提示提升了检测准确率，无需额外训练或标注数据。

Conclusion: 该方法为VLM检测系统提供了一种可扩展、模型无关的提示优化方案，替代人工提示工程。
...
### [50] [TopoDiT-3D: Topology-Aware Diffusion Transformer with Bottleneck Structure for 3D Point Cloud Generation](https://arxiv.org/abs/2505.09140)
*Zechao Guan,Feng Yan,Shuai Du,Lin Ma,Qingshan Liu*

Main category: cs.CV

TL;DR: TopoDiT-3D是一种基于拓扑感知的扩散变换器模型，通过瓶颈结构和持久同调提取全局拓扑信息，显著提升了3D点云生成的质量和效率。
...
Result: 实验表明，TopoDiT-3D在视觉质量、多样性和训练效率上优于现有方法，并验证了拓扑信息与局部特征学习的协同作用。
...
### [51] [AMSnet 2.0: A Large AMS Database with AI Segmentation for Net Detection](https://arxiv.org/abs/2505.09155)
*Yichen Shi,Zhuofu Tao,Yuhao Gao,Li Huang,Hongyang Wang,Zhiping Yu,Ting-Jung Lin,Lei He*

Main category: cs.CV

TL;DR: 本文提出了一种基于分割的新型网络检测机制，提高了对复杂或噪声电路图的识别能力，并扩展了AMSnet数据集为AMSnet 2.0。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型（MLLMs）因缺乏高质量的训练数据，难以理解电路图。现有方法依赖硬编码启发式规则，难以处理复杂或噪声电路图。

Method: 提出了一种基于分割的网络检测机制，具有高鲁棒性，并能恢复位置信息以实现电路图的数字化重建。同时扩展了AMSnet数据集。

Result: 创建了AMSnet 2.0，包含2,686个电路图及相关数据，显著优于原AMSnet的792个电路图。

Conclusion: 新方法提升了电路图识别的鲁棒性，扩展的数据集为未来研究提供了更丰富的资源。
...
### [52] [DRRNet: Macro-Micro Feature Fusion and Dual Reverse Refinement for Camouflaged Object Detection](https://arxiv.org/abs/2505.09168)
...
<details>
  <summary>Details</summary>
Motivation: 伪装目标检测的核心挑战在于目标与背景在颜色、纹理和形状上的高度相似性，导致现有方法难以兼顾全局语义信息和局部细节。

Method: DRRNet采用四阶段流程，包括全局上下文特征提取、局部细节补充、双表征融合和逆向细化模块。

Result: 实验表明，DRRNet在基准数据集上显著优于现有方法。

Conclusion: DRRNet通过多尺度特征融合和逆向细化，有效抑制背景干扰并增强目标边界连续性。
...
### [53] [UniCAD: Efficient and Extendable Architecture for Multi-Task Computer-Aided Diagnosis System](https://arxiv.org/abs/2505.09178)
*Yitao Zhu,Yuan Yin,Zhenrong Shen,Zihao Zhao,Haiyu Song,Sheng Wang,Dinggang Shen,Qian Wang*

Main category: cs.CV

TL;DR: UniCAD是一个基于预训练视觉基础模型的统一架构，用于高效处理2D和3D医学图像，仅需少量任务特定参数，并通过开源平台促进研究共享。


<details>
  <summary>Details</summary>
Motivation: 解决医学影像领域缺乏开源CAD平台以及多任务CAD系统开发资源密集的问题。

Method: 采用低秩适应策略和模块化架构，结合冻结的基础模型与可插拔专家模块。

Result: 在12个医学数据集上表现优于现有方法，准确性和部署效率均提升。

Conclusion: UniCAD为医学影像诊断提供了一个高效、可扩展的开源解决方案。
...
### [54] [Zero-shot Quantization: A Comprehensive Survey](https://arxiv.org/abs/2505.09188)
*Minjun Kim,Jaehyeon Choi,Jongkeun Lee,Wonjin Cho,U Kang*

Main category: cs.CV

TL;DR: 本文综述了零样本量化（ZSQ）方法，解决了传统量化依赖训练数据的问题，分析了现有方法的分类、动机和核心思想，并提出了未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 传统量化方法需要训练数据，但在隐私、安全或法规限制下不切实际，因此零样本量化（ZSQ）成为无需真实数据的解决方案。

Method: 论文首先定义ZSQ问题及挑战，然后根据数据生成策略分类现有方法，分析其动机、核心思想和关键点。

Result: 提供了对ZSQ方法的全面概述，并总结了现有方法的优缺点。

Conclusion: 本文是首个深入探讨ZSQ的综述，提出了未来研究方向以推动该领域发展。
...
### [55] [PDE: Gene Effect Inspired Parameter Dynamic Evolution for Low-light Image Enhancement](https://arxiv.org/abs/2505.09196)
*Tong Li,Lizhi Wang,Hansen Feng,Lin Zhu,Hua Huang*

Main category: cs.CV

TL;DR: 论文提出了一种名为参数动态演化（PDE）的方法，通过模拟基因重组和突变来优化低光图像增强（LLIE）任务中的静态参数问题，从而提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 研究发现，某些参数重置为随机值时反而能提升图像增强效果，这种现象被称为基因效应。静态参数限制了模型性能，因此需要动态调整参数以适应不同图像。

Method: 提出参数动态演化（PDE），通过参数正交生成技术模拟基因重组和突变，动态调整模型参数。

Result: 实验验证了PDE方法的有效性，能够显著提升低光图像增强的性能。

Conclusion: PDE方法通过动态演化参数解决了基因效应问题，为低光图像增强任务提供了新的解决方案。
...
### [56] [A Surrogate Model for the Forward Design of Multi-layered Metasurface-based Radar Absorbing Structures](https://arxiv.org/abs/2505.09251)
*Vineetha Joy,Aditya Anand,Nidhi,Anshuman Kumar,Amit Sethi,Hema Singh*

Main category: cs.CV

TL;DR: 论文提出了一种基于卷积神经网络（CNN）的代理模型，用于快速预测多层超表面雷达吸收结构（RAS）的电磁响应，显著减少了计算时间和设计空间探索。


<details>
  <summary>Details</summary>
Motivation: 传统方法依赖全波仿真工具设计优化超表面RAS，计算量大且耗时，需探索大设计空间。

Method: 采用基于Huber损失函数的CNN架构预测RAS的反射特性。

Result: 模型在1000次训练周期内达到99.9%的余弦相似度和0.001的均方误差，计算时间显著减少且预测精度高。

Conclusion: 该代理模型高效且准确，为超表面RAS的设计优化提供了新途径。
...
### [57] [Zero-Shot Multi-modal Large Language Model v.s. Supervised Deep Learning: A Comparative Study on CT-Based Intracranial Hemorrhage Subtyping](https://arxiv.org/abs/2505.09252)
*Yinuo Wang,Yue Zeng,Kai Chen,Cai Meng,Chao Pan,Zhouping Tang*

Main category: cs.CV

TL;DR: 研究比较了多模态大语言模型（MLLMs）与传统深度学习方法在颅内出血（ICH）分类和分型中的表现，发现传统方法在准确性上更优，但MLLMs在交互性和可解释性上有潜力。


<details>
  <summary>Details</summary>
Motivation: 及时识别非对比CT中的ICH亚型对预后预测和治疗决策至关重要，但由于低对比度和模糊边界，这一任务具有挑战性。

Method: 使用RSNA提供的192个NCCT数据集，比较了GPT-4o、Gemini 2.0 Flash、Claude 3.5 Sonnet V2等MLLMs与ResNet50、Vision Transformer等传统模型的表现。

Result: 传统模型在ICH二分类和亚型分类任务中全面优于MLLMs，Gemini 2.0 Flash的宏平均精度为0.41，F1分数为0.31。

Conclusion: MLLMs在交互性上有优势，但准确性不及传统方法。未来将优化模型以提高其在三维医学图像处理中的表现。
...
### [58] [Test-Time Augmentation for Pose-invariant Face Recognition](https://arxiv.org/abs/2505.09256)
*Jaemin Jung,Youngjoon Jang,Joon Son Chung*

Main category: cs.CV

TL;DR: Pose-TTA是一种无需额外训练的方法，通过在测试阶段生成匹配的侧脸图像来提升人脸识别性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常需要针对每个数据集重新训练或调整，而Pose-TTA旨在减少这种繁琐的工作，同时避免因正面化处理导致的信息损失。

Method: 使用肖像动画器将源图像身份转移到驱动图像的姿态中，生成匹配的侧脸图像，并提出加权特征聚合策略以减少合成数据的偏差。

Result: 在多种数据集和预训练模型上的实验表明，Pose-TTA能显著提升推理性能。

Conclusion: Pose-TTA无需重新训练或微调现有模型，易于集成到现有的人脸识别流程中。
...
### [59] [Few-Shot Anomaly-Driven Generation for Anomaly Classification and Segmentation](https://arxiv.org/abs/2505.09263)
*Guan Gui,Bin-Bin Gao,Jun Liu,Chengjie Wang,Yunsheng Wu*

Main category: cs.CV

TL;DR: 提出了一种基于少量真实异常的生成方法（AnoGen），通过扩散模型生成多样且真实的异常样本，提升异常检测模型的性能。


<details>
  <summary>Details</summary>
Motivation: 工业检测中异常样本稀缺，现有合成方法生成的异常与真实异常语义差距大，导致检测性能不佳。

Method: 分三阶段：1）学习异常分布并嵌入知识；2）利用嵌入和边界框指导扩散模型生成异常；3）提出弱监督方法训练模型。

Result: 在MVTec数据集上，DRAEM和DesTSeg的AU-PR指标分别提升了5.8%和1.5%。

Conclusion: AnoGen能有效生成真实异常，显著提升异常分类和分割任务的性能。
...
### [60] [Learning to Detect Multi-class Anomalies with Just One Normal Image Prompt](https://arxiv.org/abs/2505.09264)
*Bin-Bin Gao*
...
<details>
  <summary>Details</summary>
Motivation: 现有的自注意力重建模型在统一异常检测中表现优异，但可能因上下文一致性而无法区分正常与异常特征，且低分辨率潜在空间的重建导致异常分割不准确。

Method: 提出OneNIP方法，利用一张正常图像提示重建正常特征并恢复异常特征；同时引入监督细化器，通过真实正常与合成异常图像回归重建误差，提升像素级异常分割。

Result: OneNIP在MVTec、BTAD和VisA三个工业异常检测基准测试中优于先前方法。

Conclusion: OneNIP是一种简单有效的方法，显著提升了统一异常检测的性能和像素级分割的准确性。
...
### [61] [MetaUAS: Universal Anomaly Segmentation with One-Prompt Meta-Learning](https://arxiv.org/abs/2505.09265)
*Bin-Bin Gao*

Main category: cs.CV

TL;DR: 提出了一种基于纯视觉基础模型的通用视觉异常分割方法MetaUAS，通过统一异常分割与变化分割，利用合成数据集训练，无需依赖语言模型或特定异常数据集。


<details>
  <summary>Details</summary>
Motivation: 视觉表示与语言无关，探索纯视觉模型替代视觉-语言模型进行通用视觉异常分割的潜力。

Method: 提出MetaUAS框架，利用合成图像对训练，通过软特征对齐模块处理几何变化，实现单图像语义分割与变化感知的统一。

Result: MetaUAS在零样本、少样本甚至全样本异常分割任务中显著优于现有方法。

Conclusion: MetaUAS首次实现了不依赖语言模型和特定数据集的通用异常分割，仅需一张正常图像提示且无需训练。
...
### [62] [Recent Advances in Medical Imaging Segmentation: A Survey](https://arxiv.org/abs/2505.09274)
...
<details>
  <summary>Details</summary>
Motivation: 医学图像分割面临数据可访问性、标注复杂性、结构变异性、模态多样性和隐私限制等挑战，现有模型在泛化和领域适应方面仍有不足。

Method: 综述了生成式AI、少样本学习、基础模型和通用模型等方法，分析了其理论基础和技术实现。

Result: 这些方法为解决医学图像分割的长期挑战提供了潜在解决方案，但仍存在局限性和未解决问题。

Conclusion: 未来研究应关注提升分割模型的实用性和可访问性，同时通过GitHub仓库持续跟踪该领域创新。
...
### [63] [Predicting butterfly species presence from satellite imagery using soft contrastive regularisation](https://arxiv.org/abs/2505.09306)
*Thijs L van der Plas,Stephen Law,Michael JO Pocock*

Main category: cs.CV

TL;DR: 本文提出了一种利用卫星图像预测英国蝴蝶物种存在的新数据集，并优化了基于Resnet的模型，结合对比正则化损失方法，提高了预测准确性。


<details>
  <summary>Details</summary>
Motivation: 随着对可扩展生物多样性监测方法的需求增长，遥感数据的广泛应用和覆盖范围使其成为研究热点。传统方法主要关注栖息地映射和监测，但结合大规模公民科学野生动物观测数据，直接预测多物种存在成为新方向。

Method: 本文开发了一个新的数据集，用于从卫星图像预测蝴蝶物种存在。实验优化了基于Resnet的模型，并设计了一种针对概率标签的软监督对比正则化损失方法。

Result: 模型在物种生物多样性高的地区表现优于基线方法，对比正则化损失进一步提高了预测准确性。
...
### [64] [Neural Video Compression using 2D Gaussian Splatting](https://arxiv.org/abs/2505.09324)
...
<details>
  <summary>Details</summary>
Motivation: 传统视频编解码标准（如AVC、HEVC等）依赖手工设计的特征，而神经视频编解码（NVC）通过学习内容感知的压缩策略，提供了更高的压缩效率和适应性。然而，其高计算需求限制了实时应用（如视频会议）。本文旨在解决这一问题。

Method: 提出了一种基于ROI的神经视频压缩模型，采用2D高斯泼溅技术，结合内容感知初始化策略和帧间冗余减少机制，显著提升了编码速度。

Result: 实验表明，该模型将基于高斯泼溅的图像编解码器的编码时间缩短了88%，并首次实现了高斯泼溅在视频编解码中的应用。

Conclusion: 该方法为神经视频编解码领域提供了首个基于高斯泼溅的解决方案，具有实时应用的潜力。
...
### [65] [BioVFM-21M: Benchmarking and Scaling Self-Supervised Vision Foundation Models for Biomedical Image Analysis](https://arxiv.org/abs/2505.09329)
*Jiarun Liu,Hong-Yu Zhou,Weijian Huang,Hao Yang,Dongning Song,Tao Tan,Yong Liang,Shanshan Wang*

Main category: cs.CV

TL;DR: 论文研究了医学视觉基础模型的扩展行为，发现扩展模型和数据规模对性能有提升，但效果因任务而异。提出了BioVFM-21M数据集和BioVFM模型，在12个医学基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 医学图像与自然数据差异显著，缺乏对医学领域扩展行为的深入理解，阻碍了医学视觉基础模型的开发。

Method: 通过自监督学习，研究了模型规模、训练算法、数据规模和成像模态的扩展行为，并提出了BioVFM-21M数据集和BioVFM模型。

Result: BioVFM模型在12个医学基准测试中优于现有基础模型，扩展规模对性能有提升但效果因任务而异。

Conclusion: 扩展规模对医学基础模型有益，但任务特性、数据多样性、预训练方法和计算效率仍需重点考虑。
...
### [66] [Unsupervised Multiview Contrastive Language-Image Joint Learning with Pseudo-Labeled Prompts Via Vision-Language Model for 3D/4D Facial Expression Recognition](https://arxiv.org/abs/2505.09336)
*Muzammil Behzad*
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在多视角情绪表示学习中缺乏无监督的隐式对齐能力，且对比学习的稳定性和可扩展性不足。

Method: 模型结合伪标签引导情绪语义对齐，提出联合嵌入空间和多视角对比学习策略，并引入梯度友好损失函数和分布式训练优化。

Result: 实验表明MultiviewVLM优于现有方法，且易于适应实际应用。

Conclusion: MultiviewVLM为多视角情绪表示学习提供了高效、可扩展的解决方案。
...
### [67] [Marigold: Affordable Adaptation of Diffusion-Based Image Generators for Image Analysis](https://arxiv.org/abs/2505.09358)
*Bingxin Ke,Kevin Qu,Tianfu Wang,Nando Metzger,Shengyu Huang,Bo Li,Anton Obukhov,Konrad Schindler*

Main category: cs.CV

TL;DR: Marigold是一种基于预训练潜在扩散模型（如Stable Diffusion）的条件生成模型家族，通过微调协议将其知识提取并应用于密集图像分析任务（如深度估计、表面法线预测等），实现了零样本泛化的最先进性能。


<details>
  <summary>Details</summary>
Motivation: 在数据稀缺的情况下，预训练模型的质量对迁移学习至关重要。文本到图像生成模型（如潜在扩散模型）展示了强大的视觉理解能力，但其在密集图像分析任务中的应用尚未充分探索。

Method: Marigold通过对预训练潜在扩散模型进行最小架构修改，利用小规模合成数据集在单GPU上进行微调，提取其知识并适应密集图像分析任务。

Result: Marigold在单GPU上训练几天即可实现零样本泛化的最先进性能，适用于深度估计、表面法线预测和本征分解等任务。

Conclusion: Marigold展示了预训练潜在扩散模型在密集图像分析任务中的潜力，为数据稀缺场景下的迁移学习提供了高效解决方案。
...
### [68] [RobustSpring: Benchmarking Robustness to Image Corruptions for Optical Flow, Scene Flow and Stereo](https://arxiv.org/abs/2505.09368)
*Jenny Schmalfuss,Victor Oei,Lukas Mehl,Madlen Bartsch,Shashank Agnihotri,Margret Keuper,Andrés Bruhn*

Main category: cs.CV

TL;DR: RobustSpring是一个新的数据集和基准测试，用于评估光流、场景流和立体视觉模型对图像损坏的鲁棒性，填补了现有基准测试在模型鲁棒性评估上的空白。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要关注模型准确性，而忽略了模型对噪声、雨等现实世界扰动的鲁棒性，导致模型在这些条件下的表现无法量化。

Method: 通过在高分辨率Spring数据集上应用20种不同的图像损坏（如噪声、模糊、颜色变化等），生成20,000张损坏图像，并设计新的鲁棒性度量标准。

Result: 实验表明，准确性高的模型不一定鲁棒，且鲁棒性因损坏类型而异。

Conclusion: RobustSpring将鲁棒性作为首要考量，旨在推动兼具准确性和鲁棒性的模型发展。
...
### [69] [MAKE: Multi-Aspect Knowledge-Enhanced Vision-Language Pretraining for Zero-shot Dermatological Assessment](https://arxiv.org/abs/2505.09372)
...
<details>
  <summary>Details</summary>
Motivation: 皮肤病诊断需要结合视觉特征和临床知识，但现有视觉语言预训练方法因文本长度限制和非结构化文本而效果有限。

Method: 1. 多视角对比学习分解临床叙述；2. 细粒度对齐子标题与图像特征；3. 诊断引导加权优先临床重要子标题。

Result: 在403,563个图像-文本对上预训练后，MAKE在8个数据集上零样本任务表现显著优于现有方法。
...
### [70] [Text-driven Motion Generation: Overview, Challenges and Directions](https://arxiv.org/abs/2505.09379)
*Ali Rida Sahili,Najett Neji,Hedi Tabia*

Main category: cs.CV

TL;DR: 本文综述了基于文本驱动的人体运动生成方法，探讨了其架构和运动表示，并总结了数据集、评估方法和未来方向。


<details>
  <summary>Details</summary>
Motivation: 文本驱动运动生成提供了一种直观且灵活的方式，适用于虚拟现实、游戏、人机交互和机器人等领域。

Method: 从架构（VAE、扩散模型、混合模型）和运动表示（离散与连续）两个角度分类现代文本到运动生成方法。

Result: 总结了当前领域的主要方法、数据集和评估标准，并指出了关键挑战和未来方向。

Conclusion: 本文为语言驱动运动合成的研究者和实践者提供了有价值的参考，并展望了未来的研究方向。
...
### [71] [Examining Deployment and Refinement of the VIOLA-AI Intracranial Hemorrhage Model Using an Interactive NeoMedSys Platform](https://arxiv.org/abs/2505.09380)
*Qinghui Liu,Jon Nesvold,Hanna Raaum,Elakkyen Murugesu,Martin Røvang,Bradley J Maclntosh,Atle Bjørnerud,Karoline Skogen*

Main category: cs.CV

TL;DR: 研究描述了一个名为NeoMedSys的放射学软件平台，用于高效部署和优化AI模型，并在真实临床环境中评估其可行性。通过迭代改进，AI模型（VIOLA-AI）在颅内出血检测中的性能显著提升。


<details>
  <summary>Details</summary>
Motivation: 解决AI工具在放射学临床部署中的挑战，并探索如何通过平台支持AI模型的实时优化。

Method: NeoMedSys整合了AI模型部署、测试和优化工具，结合医疗图像查看器和注释系统，在挪威最大急诊科进行临床案例研究。

Result: VIOLA-AI的敏感性和特异性显著提高（敏感性从79.2%升至90.3%，特异性从80.7%升至89.3%），AUC从0.873提升至0.949。

Conclusion: NeoMedSys通过实时反馈和迭代优化显著提升了AI模型的性能，证明了其在临床环境中的价值。
...
### [72] [FedSaaS: Class-Consistency Federated Semantic Segmentation via Global Prototype Supervision and Local Adversarial Harmonization](https://arxiv.org/abs/2505.09385)
*Xiaoyang Yu,Xiaoming Wu,Xin Wang,Dongrun Li,Ming Yang,Peng Cheng*

Main category: cs.CV

TL;DR: 提出了一种名为FedSaaS的新联邦语义分割框架，通过类样本和对比损失解决类一致性问题，显著提升了分割精度。


<details>
  <summary>Details</summary>
Motivation: 现有研究在解决异构问题时忽略了语义空间中的细粒度类关系，导致类表示模糊。

Method: 引入类样本作为本地和全局类表示的准则，服务器端建模类原型监督客户端全局分支，客户端通过对抗机制协调全局和本地分支，并使用多级对比损失。
...
### [73] [FreeDriveRF: Monocular RGB Dynamic NeRF without Poses for Autonomous Driving via Point-Level Dynamic-Static Decoupling](https://arxiv.org/abs/2505.09406)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖精确姿态输入和多传感器数据，系统复杂。FreeDriveRF旨在简化系统，仅用RGB图像实现动态场景重建。

Method: 通过语义监督解耦动态与静态部分，引入光流约束动态建模，并利用动态流优化姿态估计。

Result: 在KITTI和Waymo数据集上表现优异，动态场景建模效果显著。

Conclusion: FreeDriveRF为自动驾驶动态场景重建提供了一种高效且无需姿态输入的解决方案。
...
### [74] [Sparse Point Cloud Patches Rendering via Splitting 2D Gaussians](https://arxiv.org/abs/2505.09413)
*Ma Changfeng,Bi Ran,Guo Jie,Wang Chongjun,Guo Yanwen*

Main category: cs.CV

TL;DR: 提出了一种基于点云预测2D高斯的新渲染方法，无需依赖类别先验、密集点云或额外优化，适用于稀疏点云并实现跨类别泛化。


<details>
  <summary>Details</summary>
Motivation: 现有基于学习的方法依赖类别先验或密集点云，限制了泛化能力，因此提出一种更灵活的点云渲染方法。

Method: 采用双模块架构，通过点云信息（法线、颜色、距离）初始化高斯分布，利用分裂解码器优化初始高斯分布，适应稀疏点云。

Result: 实验证明该方法在多个数据集上表现优异，实现跨类别泛化，达到SOTA性能。

Conclusion: 该方法无需额外优化即可直接渲染，保留了2D高斯的优势，具有广泛适用性。
...
### [75] [FaceShield: Explainable Face Anti-Spoofing with Multimodal Large Language Models](https://arxiv.org/abs/2505.09415)
*Hongyang Wang,Yichen Shi,Zhuofu Tao,Yuhao Gao,Liepiao Zhang,Xun Lin,Jun Feng,Xiaochen Yuan,Zitong Yu,Xiaochun Cao*

Main category: cs.CV

TL;DR: FaceShield是一种多模态大语言模型（MLLM），专为面部反欺骗（FAS）设计，提供分类、推理和攻击区域检测功能，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有FAS方法缺乏可解释性，且缺乏专为FAS设计的通用MLLM和数据集。

Method: 结合原始图像和先验知识的辅助信息，采用提示引导的视觉标记掩码策略（PVTM）提升泛化能力。

Result: 在三个基准数据集上，FaceShield在粗/细粒度分类、推理和攻击定位任务中表现优异。

Conclusion: FaceShield为FAS任务提供了通用解决方案，未来将公开数据集和代码。
...
### [76] [MoRAL: Motion-aware Multi-Frame 4D Radar and LiDAR Fusion for Robust 3D Object Detection](https://arxiv.org/abs/2505.09422)
*Xiangyuan Peng,Yu Wang,Miao Tang,Bierzynski Kay,Lorenzo Servadei,Robert Wille*

Main category: cs.CV

TL;DR: MoRAL提出了一种基于运动感知的多帧4D雷达与LiDAR融合框架，用于鲁棒的3D物体检测，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法忽略了雷达点云在帧间的错位问题，且未充分利用4D雷达的动态信息。

Method: 设计了运动感知雷达编码器（MRE）补偿运动物体的帧间错位，并通过运动注意力门控融合（MAGF）模块整合雷达动态信息。

Result: 在VoD数据集上，MoRAL在整体区域和驾驶走廊分别达到73.30%和88.68%的mAP，对行人和骑行者的检测表现尤为突出。

Conclusion: MoRAL通过运动感知和多模态融合显著提升了3D物体检测的准确性。
...
### [77] [Efficient LiDAR Reflectance Compression via Scanning Serialization](https://arxiv.org/abs/2505.09433)
*Jiahao Zhu,Kang You,Dandan Ding,Zhan Ma*

Main category: cs.CV

TL;DR: SerLiC是一种基于序列化的神经压缩框架，用于高效压缩LiDAR点云的反射率数据，通过扫描顺序序列化和Mamba模型实现高压缩率和快速处理。


<details>
  <summary>Details</summary>
Motivation: LiDAR点云的反射率属性在下游任务中很重要，但在神经压缩方法中尚未充分探索。

Method: 将3D LiDAR点云通过扫描顺序序列化为1D序列，结合传感器扫描索引、径向距离和先验反射率进行上下文表示，并使用Mamba模型进行高效序列建模。

Result: SerLiC实现了超过2倍的体积压缩，比现有方法减少22%的压缩比特，参数仅占2%。轻量版达到>10 fps，仅需111K参数。
...
TL;DR: Endo-CLIP是一个自监督框架，通过三阶段处理（清洗、调整和统一）改进内窥镜图像分析，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决内窥镜图像分析中的非信息背景图像、复杂医学术语和多病变描述模糊问题。

Method: 三阶段框架：清洗（去除背景帧）、调整（利用大语言模型提取临床属性）、统一（患者级跨注意力解决多息肉模糊）。

Result: 在零样本和少样本息肉检测与分类中显著优于现有方法。
...
### [79] [MrTrack: Register Mamba for Needle Tracking with Rapid Reciprocating Motion during Ultrasound-Guided Aspiration Biopsy](https://arxiv.org/abs/2505.09450)
*Yuelin Zhang,Qingpeng Ding,Long Lei,Yongxuan Feng,Raymond Shing-Yan Tang,Shing Shin Cheng*

Main category: cs.CV

TL;DR: MrTrack是一种基于Mamba的超声引导细针穿刺活检针追踪器，通过全局上下文提取和时序提示检索，解决了快速往复运动导致的视觉特征不可用问题，实验表明其在准确性和效率上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 超声引导细针穿刺活检（FNA）中，快速往复运动导致现有追踪器无法有效工作，亟需一种新方法解决这一问题。

Method: 提出MrTrack，利用Mamba机制提取全局上下文并存储在寄存器库中，通过时序提示检索提供外部线索，同时引入自监督损失防止特征崩溃。

Result: 在自动和手动穿刺数据集上的实验表明，MrTrack在准确性、鲁棒性和推理效率上均优于现有追踪器。

Conclusion: MrTrack通过创新的寄存器机制和自监督损失，显著提升了超声引导FNA活检针追踪的性能。
...
### [80] [Beyond Pixels: Leveraging the Language of Soccer to Improve Spatio-Temporal Action Detection in Broadcast Videos](https://arxiv.org/abs/2505.09455)
*Jeremie Ochin,Raphael Chekroun,Bogdan Stanciulescu,Sotiris Manitsaris*

Main category: cs.CV

TL;DR: 论文提出了一种基于Transformer的编码器-解码器模型，通过结合游戏状态信息和上下文理解，改进时空动作检测（STAD）方法，减少足球视频分析中的误报。


<details>
  <summary>Details</summary>
Motivation: 现有STAD方法在高召回率、低精度要求下缺乏上下文理解，导致大量误报，需要结合更广泛的行动序列和游戏状态信息。

Method: 使用Transformer模型处理噪声的球员中心预测序列和干净的比赛状态信息，通过建模长时间上下文和团队动态，生成“去噪”动作序列。

Result: 该方法在低置信度区域提高了精度和召回率，实现了更可靠的足球事件提取。

Conclusion: 通过结合游戏级推理和序列去噪任务，该方法显著提升了STAD在足球视频分析中的性能。
...
### [81] [A 2D Semantic-Aware Position Encoding for Vision Transformers](https://arxiv.org/abs/2505.09466)
...
TL;DR: 论文提出了一种新的2D语义感知位置编码方法（SaPE²），解决了传统位置编码在视觉任务中无法有效捕捉语义关系的问题，提升了模型的泛化能力和性能。


<details>
  <summary>Details</summary>
Motivation: 传统的位置编码方法（如绝对和相对位置编码）主要关注1D线性位置关系，忽略了图像块之间的语义相似性，限制了模型的泛化能力和翻译等变性。

Method: 提出2D语义感知位置编码（SaPE²），通过动态调整位置表示，利用局部内容而非固定线性关系或空间坐标，增强语义感知。

Result: SaPE²提升了模型在不同图像分辨率和尺度下的泛化能力，改善了翻译等变性，并更好地聚合了视觉相似但空间距离较远的图像块特征。

Conclusion: 通过将SaPE²集成到视觉Transformer中，弥补了位置编码与感知相似性之间的差距，显著提升了计算机视觉任务的性能。
...
### [82] [Denoising and Alignment: Rethinking Domain Generalization for Multimodal Face Anti-Spoofing](https://arxiv.org/abs/2505.09484)
*Yingjie Ma,Xun Lin,Zitong Yu,Xin Liu,Xiaochen Yuan,Weicheng Xie,Linlin Shen*

Main category: cs.CV

TL;DR: MMDA框架通过去噪和对齐机制提升多模态人脸防伪的泛化性能，MD2A模块和RS2策略进一步优化噪声抑制和表示对齐，实验证明其优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决多模态人脸防伪（FAS）中模态特定偏差和域偏移导致的泛化能力不足问题。

Method: 提出MMDA框架，结合CLIP的零样本泛化能力，通过MD2A模块抑制噪声，RS2策略对齐表示空间，并设计U-DSA模块增强适应性。
...
### [83] [Flash-VL 2B: Optimizing Vision-Language Model Performance for Ultra-Low Latency and High Throughput](https://arxiv.org/abs/2505.09498)
*Bo Zhang,Shuo Li,Runhe Tian,Yang Yang,Jixin Tang,Jinhao Zhou,Lin Ma*

Main category: cs.CV

TL;DR: Flash-VL 2B是一种优化视觉语言模型的新方法，旨在实现超低延迟和高吞吐量，同时保持准确性。


<details>
  <summary>Details</summary>
Motivation: 针对实时应用需求，优化视觉语言模型的性能，使其在资源受限环境中高效运行。
...
### [84] [Conformal Bounds on Full-Reference Image Quality for Imaging Inverse Problems](https://arxiv.org/abs/2505.09528)
...
<details>
  <summary>Details</summary>
Motivation: 在医学成像等安全关键应用中，准确评估恢复图像与真实图像的质量差异至关重要，但传统方法因缺乏真实图像而难以实现。

Method: 结合共形预测和近似后验采样，构建FRIQ指标的统计边界，确保其在一定误差概率内可靠。

Result: 在图像去噪和加速MRI问题上验证了方法的有效性，代码已开源。

Conclusion: 该方法为FRIQ评估提供了可靠的统计保证，适用于安全关键领域。
...
### [85] [Contactless Cardiac Pulse Monitoring Using Event Cameras](https://arxiv.org/abs/2505.09529)
*Mohamed Moustafa,Joseph Lemley,Peter Corcoran*

Main category: cs.CV

TL;DR: 该研究利用事件相机和CNN模型，从面部事件流中无接触重建心率信号，结果表明事件相机在高帧率下优于传统相机。


<details>
  <summary>Details</summary>
Motivation: 探索事件相机在无接触心率监测中的潜力，利用其高动态范围和高时间分辨率特性。

Method: 使用监督学习的CNN模型，从事件流中提取心率信号，并与传统相机结果对比。
...
### [86] [Camera-Only 3D Panoptic Scene Completion for Autonomous Driving through Differentiable Object Shapes](https://arxiv.org/abs/2505.09562)
*Nicola Marinello,Simen Cassiman,Jonas Heylen,Marc Proesmans,Luc Van Gool*

Main category: cs.CV

TL;DR: 论文提出了一种用于3D全景场景补全的新框架，扩展了现有3D语义场景补全模型，通过对象模块和全景模块提升路径规划和决策能力。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶车辆需要完整的环境地图以规划和行动，而3D全景场景补全任务目前研究不足，但对其路径规划和决策至关重要。

Method: 提出了一种新框架，包含对象模块和全景模块，可轻松与现有3D占用和场景补全方法集成，利用占用基准中的注释学习对象形状。

Result: 该方法能够预测密集的体素网格地图，并区分同一类别中的对象实例，提升场景补全能力。
...
### [87] [Using Foundation Models as Pseudo-Label Generators for Pre-Clinical 4D Cardiac CT Segmentation](https://arxiv.org/abs/2505.09564)
...
TL;DR: 论文探讨了利用基础模型为猪心脏CT生成伪标签，并通过自训练方法迭代优化这些标签，无需手动标注数据。


<details>
  <summary>Details</summary>
Motivation: 由于猪与人类心脏的相似性，猪模型在临床前研究中常用，但物种差异导致模型直接迁移困难。研究旨在探索基础模型在猪心脏CT分割中的适用性。

Method: 提出了一种基于基础模型生成伪标签的自训练方法，通过迭代优化提高分割质量，无需人工标注数据。

Result: 自训练方法不仅提高了分割精度，还减少了连续帧间的时间不一致性。

Conclusion: 虽然结果令人鼓舞，但仍有改进空间，如采用更复杂的自训练策略或探索其他基础模型和成像技术。
...
### [88] [BLIP3-o: A Family of Fully Open Unified Multimodal Models-Architecture, Training and Dataset](https://arxiv.org/abs/2505.09568)
...
TL;DR: 论文提出了一种基于扩散变换器的统一多模态模型BLIP3-o，结合自回归和扩散模型的优势，优化了图像生成与理解的统一框架，并通过创新的训练策略和高质量数据集实现了卓越性能。


<details>
  <summary>Details</summary>
Motivation: 统一图像理解与生成是多模态模型研究的热点，但现有方法在模型架构和训练策略上仍有不足。本文旨在探索自回归和扩散模型在统一框架中的潜力。

Method: 采用扩散变换器生成CLIP图像特征，提出分阶段预训练策略（先图像理解后图像生成），并构建高质量指令调优数据集BLIP3o-60k。

Result: BLIP3-o在图像理解和生成任务中均表现优异，训练效率高且生成质量提升。

Conclusion: BLIP3-o通过创新的模型设计、训练策略和数据集，为统一多模态模型研究提供了新方向，并开源了相关资源。
...
### [89] [Don't Forget your Inverse DDIM for Image Editing](https://arxiv.org/abs/2505.09571)
*Guillermo Gomez-Trenado,Pablo Mesejo,Oscar Cordón,Stéphane Lathuilière*

Main category: cs.CV

TL;DR: SAGE是一种基于预训练扩散模型的新图像编辑技术，通过自注意力机制优化编辑效率和质量。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像生成方法在编辑真实图像时存在计算量大或重建效果差的问题。

Method: SAGE基于DDIM算法，利用扩散U-Net的自注意力层设计新引导机制，通过注意力图计算重建目标。

Result: SAGE在定量和定性评估中优于其他方法，用户研究中47名用户均偏好SAGE，并在10项定量分析中7项排名第一。

Conclusion: SAGE有效解决了图像编辑中的关键挑战，展现了卓越的性能和用户满意度。
...
### [90] [Variational Visual Question Answering](https://arxiv.org/abs/2505.09591)
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态模型在视觉问答（VQA）中存在校准不足和可靠性问题，尤其是在分布偏移（OOD）情况下。现有研究主要针对单模态模型，多模态模型的研究较少。

Method: 采用变分算法IVON替代传统的AdamW优化器，生成模型参数的后验分布，以提升模型的校准性和可靠性。

Result: 实验表明，该方法在保持AdamW准确性的同时，显著降低了预期校准误差（>50%），并提高了覆盖率（4% vs. SOTA）。在分布偏移情况下，性能提升更显著（覆盖率提升8% vs. SOTA）。
...
### [91] [LightLab: Controlling Light Sources in Images with Diffusion Models](https://arxiv.org/abs/2505.09608)
...
<details>
  <summary>Details</summary>
Motivation: 现有重光照方法要么依赖多视图输入进行逆渲染，要么无法提供对光源变化的显式控制。

Method: 通过在少量真实原始照片对和合成渲染图像上微调扩散模型，利用光的线性特性合成图像对，训练模型以实现精确的光照变化控制。

Result: 该方法能够实现引人注目的光照编辑效果，并在用户偏好上优于现有方法。

Conclusion: 该方法通过微调扩散模型，实现了对光源强度和颜色的显式控制，为光照编辑提供了新思路。
...
### [92] [UWAV: Uncertainty-weighted Weakly-supervised Audio-Visual Video Parsing](https://arxiv.org/abs/2505.09615)
...
<details>
  <summary>Details</summary>
Motivation: AVVP任务需要定位单模态和多模态事件，但标注成本高，限制了可扩展性。现有方法在生成伪标签时缺乏段间依赖性和存在预测偏差。

Method: 提出UWAV方法，利用不确定性加权伪标签和特征混合正则化改进训练。

Result: UWAV在多个指标和数据集上优于现有方法。
...
### [93] [A Preliminary Framework for Intersectionality in ML Pipelines](https://arxiv.org/abs/2505.08792)
...
<details>
  <summary>Details</summary>
Motivation: 机器学习技术未能充分支持复杂的社会身份和体验，交叉性理论为解决这一问题提供了社会学基础。

Method: 通过放大Crenshaw、Combahee和Collins的交叉性理论，构建初步框架，并评估机器学习文献中的应用情况。

Result: 发现机器学习文献中对交叉性理论的应用存在偏差，削弱了其潜在影响。

Conclusion: 呼吁正确应用交叉性理论，以实现更公平的技术成果。
...
### [94] [Onboard Optimization and Learning: A Survey](https://arxiv.org/abs/2505.08793)
...
Result: 通过硬件-软件协同设计、模型压缩和去中心化学习，提供了当前板载学习的现状。
...
### [95] [The Geometry of Meaning: Perfect Spacetime Representations of Hierarchical Structures](https://arxiv.org/abs/2505.08795)
*Andres Anabalon,Hugo Garces,Julio Oliva,Jose Cifuentes*

Main category: cs.LG

TL;DR: 论文提出了一种快速算法，将层次结构嵌入三维闵可夫斯基时空，数据相关性完全编码在因果结构中。方法仅依赖局部层次信号，无需全局符号结构，并在WordNet语料库中验证了完美嵌入。


<details>
  <summary>Details</summary>
Motivation: 探索离散数据（如层次结构）在几何空间中的完美表示，揭示层次意义本身的几何本质。

Method: 使用定向令牌对（局部层次信号）构建模型，无需全局符号结构，通过因果结构编码层次关系。

Result: 成功嵌入WordNet的哺乳动物子树和最大无歧义子集（82,115个名词），层次结构完全由几何编码。

Conclusion: 离散数据可在三维几何中完美表示，层次意义本质上是几何的，与广义相对论和场论有深层联系。
...
### [96] [Multi-modal Synthetic Data Training and Model Collapse: Insights from VLMs and Diffusion Models](https://arxiv.org/abs/2505.08803)
*Zizhao Hu,Mohammad Rostami,Jesse Thomason*

Main category: cs.LG

TL;DR: 研究了多模态生成模型在自生成数据训练中的崩溃风险，发现其特性与单模态模型不同，并提出缓解方法。


<details>
  <summary>Details</summary>
Motivation: 探索多模态生成模型在自生成数据训练中的崩溃现象，以填补单模态研究的不足。

Method: 扩展研究至多模态视觉-语言生成系统，如视觉-语言模型和文本-图像扩散模型，并进行递归生成-训练循环。

Result: 多模态模型崩溃表现出独特特性，如视觉-语言对齐改善和任务方差增加，并提出缓解方法。

Conclusion: 为多模态自改进AI系统提供了初步见解和实用指南，以减少模型崩溃风险。
...
### [97] [An Extra RMSNorm is All You Need for Fine Tuning to 1.58 Bits](https://arxiv.org/abs/2505.08823)
*Cody Steinmetz,Gavin Childress,Aaron Herbst,Gavin Jones,Jasdeep Singh,Eli Vang,Keagan Weinstock*

Main category: cs.LG

TL;DR: 论文提出了一种通过RMS归一化和渐进式量化方法，将全精度LLM稳定微调为三元（2位）模型的技术，无需额外训练或增加模型复杂度，性能接近全精度模型。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLM）部署成本高，量化虽能减少资源消耗但常导致精度下降，三元量化更不稳定，亟需一种稳定高效的量化方法。

Method: 在每层线性投影前插入RMS归一化，并采用渐进式分层量化策略，直接微调全精度模型为三元LLM。

Result: 该方法在标准语言建模基准上表现优于复杂知识蒸馏流程，且无需增加模型复杂度。

Conclusion: 研究表明，仅通过精心设计的归一化即可显著缩小三元与全精度LLM的精度差距，使超低比特推理实用化。
...
### [98] [Self Rewarding Self Improving](https://arxiv.org/abs/2505.08827)
*Toby Simonds,Kevin Lopez,Akira Yoshiyama,Dominique Garmier*

Main category: cs.LG

TL;DR: 大型语言模型通过自我评判实现自我改进，无需参考答案，利用生成与验证的不对称性。实验表明模型能提供可靠奖励信号，结合合成问题生成实现完整自改进循环，性能提升显著。


<details>
  <summary>Details</summary>
Motivation: 探索语言模型在无监督下自我改进的可能性，解决传统强化学习中奖励信号难以获取的问题。

Method: 通过自我评判和合成问题生成，模型生成问题、解决并评估自身表现，形成自改进循环。

Result: 在Countdown谜题和MIT积分问题上，模型性能提升8%，超越GPT-4o。

Conclusion: 自我评判为强化学习提供有效奖励信号，推动AI系统通过自学习持续改进，适用于数据稀缺或复杂评估领域。
...
### [99] [Aggregating Concepts of Fairness and Accuracy in Predictive Systems](https://arxiv.org/abs/2505.08829)
*David Kinney*

Main category: cs.LG

TL;DR: 论文提出用线性组合方法衡量预测算法的综合价值，解决准确性与公平性之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 随着AI预测算法的强大，如何在准确性与公平性之间找到平衡成为重要挑战，缺乏明确的规范性指导。

Method: 采用Harsanyi的偏好聚合理论，提出线性组合方法衡量算法的综合价值。

Result: 通过COMPAS数据集验证了线性组合方法的有效性。

Conclusion: 线性组合方法为解决准确性与公平性之间的权衡提供了可行的理论框架。
...
### [100] [Evaluating Simplification Algorithms for Interpretability of Time Series Classification](https://arxiv.org/abs/2505.08846)
*Felix Marti-Perez,Brigt Håvardstun,Cèsar Ferri,Carlos Monserrat,Jan Arne Telle*

Main category: cs.LG

TL;DR: 论文提出了一种评估简化时间序列在时间序列分类器（TSC）可解释性中的方法，通过复杂性和忠诚度指标比较了四种简化算法。


<details>
  <summary>Details</summary>
Motivation: 时间序列数据对人类不直观，简化有助于提高TSC的可解释性。

Method: 引入复杂性和忠诚度指标，评估四种简化算法在不同TSC算法和数据集上的表现。

Result: 简化时间序列在季节性、非平稳和低熵数据中显著优于原始时间序列。

Conclusion: 简化时间序列能有效提升TSC的可解释性，尤其在特定数据特征下表现更佳。
...
### [101] [An Analytical Characterization of Sloppiness in Neural Networks: Insights from Linear Models](https://arxiv.org/abs/2505.08915)
*Jialin Mao,Itay Griniasty,Yan Sun,Mark K. Transtrum,James P. Sethna,Pratik Chaudhari*

Main category: cs.LG

TL;DR: 论文分析了深度神经网络训练轨迹在概率分布空间中形成的低维“超带状”流形现象，并通过线性网络的理论分析揭示了其几何特性。


<details>
  <summary>Details</summary>
Motivation: 受深度网络与线性网络训练轨迹相似性的启发，研究旨在通过理论分析揭示低维流形的形成机制及其控制因素。

Method: 利用动态系统理论工具，分析输入相关矩阵特征值衰减率、初始权重与真实输出的相对尺度以及梯度下降步数对低维流形几何的影响。

Result: 确定了超带状流形出现的相边界条件，并将分析扩展至核机器和随机梯度下降训练的线性模型。

Conclusion: 研究为理解深度神经网络训练轨迹的低维流形现象提供了理论支持，并揭示了其控制因素。
...
### [102] [NeurIPS 2024 Ariel Data Challenge: Characterisation of Exoplanetary Atmospheres Using a Data-Centric Approach](https://arxiv.org/abs/2505.08940)
...
Method: 采用了多种实验方法，包括特征提取、信号变换和异方差不确定性建模，重点关注高斯对数似然（GLL）分数。

Result: 实验显示不确定性估计对GLL分数有显著影响，性能提升了11%，但也揭示了表格建模和特征工程的局限性。

Conclusion: 研究强调了模型简洁性、可解释性和泛化能力在天体物理数据分析中的权衡，并指出商业驱动方法在竞赛框架中的约束。
...
### [103] [ForeCite: Adapting Pre-Trained Language Models to Predict Future Citation Rates of Academic Papers](https://arxiv.org/abs/2505.08941)
...
Method: 通过为预训练因果语言模型添加线性头，用于平均每月引用率预测，并适应回归任务。

Result: 在90万+生物医学论文数据集上，测试相关性达到ρ=0.826，比现有方法提升27点。
...
### [104] [GPML: Graph Processing for Machine Learning](https://arxiv.org/abs/2505.08964)
*Majed Jaber,Julien Michel,Nicolas Boutry,Pierre Parrend*

Main category: cs.LG

TL;DR: GPML库通过将原始网络流量转化为图表示，提供动态网络中异常检测和社区变化分析的工具，支持实时检测和历史取证。


<details>
  <summary>Details</summary>
Motivation: 动态网络中复杂、多步骤且快速演变的攻击增加，需要先进的网络威胁检测工具。

Method: GPML库将原始网络流量转化为图表示，支持社区和谱度量提取。

Result: 该库能够检测动态网络中的异常和社区变化，增强实时检测和历史取证能力。

Conclusion: GPML以基于图的强大方法应对现代网络安全挑战。
...
### [105] [SaFARi: State-Space Models for Frame-Agnostic Representation](https://arxiv.org/abs/2505.08977)
*Hossein Babaei,Mel White,Sina Alemohammad,Richard G. Baraniuk*

Main category: cs.LG

TL;DR: 论文提出了一种广义方法SaFARi，用于构建基于任意框架或基的SSM，突破了现有方法仅限多项式基的限制。


<details>
  <summary>Details</summary>
Motivation: 现有SSM方法仅探索了少数多项式基，限制了其应用潜力。

Method: 提出SaFARi框架，支持任意框架或基的SSM构建，扩展了HiPPO方法。

Result: 该方法为SSM架构提供了无限多样的可能性。

Conclusion: SaFARi为SSM的灵活性和应用范围提供了新的可能性。
...
### [106] [Model-free Online Learning for the Kalman Filter: Forgetting Factor and Logarithmic Regret](https://arxiv.org/abs/2505.08982)
*Jiachen Qian,Yang Zheng*

Main category: cs.LG

TL;DR: 论文提出一种基于指数遗忘的在线预测方法，用于解决未知线性随机系统的预测问题，通过平衡回归模型提高预测精度，并提供了更优的对数遗憾界。
...
Method: 通过指数遗忘注入归纳偏置，平衡回归模型，优化回归与正则化误差的权衡，并减少累积误差。

Result: 实现了更优的预测精度，并提供了O(log^3 N)的对数遗憾界。

Conclusion: 提出的方法在未知线性随机系统中表现优异，平衡了回归与正则化误差，同时降低了累积误差。
...
### [107] [Ranking-Based At-Risk Student Prediction Using Federated Learning and Differential Features](https://arxiv.org/abs/2505.09287)
*Shunsuke Yoneda,Valdemar Švábenský,Gen Li,Daisuke Deguchi,Atsushi Shimada*

Main category: cs.LG

TL;DR: 该研究提出了一种结合联邦学习和差分特征的方法，以解决数字教科书学习日志数据跨校整合时的隐私问题，同时提升模型性能和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 数字教科书的学习日志数据在跨校整合时面临隐私问题，导致分析局限于单一学校数据，难以开发高性能和泛化性强的模型。

Method: 结合联邦学习和差分特征，联邦学习避免数据集中化以保护隐私，差分特征通过相对值提升模型性能。

Result: 实验表明，该方法在隐私保护的同时，性能与集中式学习相当，且差分特征在所有评估数据集上均优于非差分方法。

Conclusion: 该方法有效解决了隐私问题，提升了模型性能和泛化能力，适用于早期预测高风险学生。
...
### [108] [Continual Reinforcement Learning via Autoencoder-Driven Task and New Environment Recognition](https://arxiv.org/abs/2505.09003)
*Zeki Doruk Erden,Donia Gasmi,Boi Faltings*

Main category: cs.LG

TL;DR: 研究探索了自动编码器在强化学习持续学习中的有效性，通过结合策略优化和熟悉度自动编码器，实现了无需外部信号的任务识别与知识保留。


<details>
  <summary>Details</summary>
Motivation: 持续学习在强化学习中面临挑战，尤其是在没有外部信号指示任务或环境变化时如何保留和利用已有信息。

Method: 提出一种端到端持续学习系统，整合策略优化和熟悉度自动编码器，用于检测新任务并匹配已知环境。

Result: 初步结果显示，系统能在无外部信号的情况下成功实现持续学习，有效识别新任务并保留旧知识。

Conclusion: 该方法展示了在持续学习中的潜力，特别是在无外部信号支持的任务识别和知识保留方面。
...
### [109] [Signal-based AI-driven software solution for automated quantification of metastatic bone disease and treatment response assessment using Whole-Body Diffusion-Weighted MRI (WB-DWI) biomarkers in Advanced Prostate Cancer](https://arxiv.org/abs/2505.09011)
...
<details>
  <summary>Details</summary>
Motivation: 为临床决策提供可重复的转移性骨病监测工具。

Method: 结合弱监督Residual U-Net、统计框架和浅层卷积神经网络，生成骨病变掩膜并提取定量指标。

Result: Dice分数0.6，表面距离2mm，重复性变异系数低于5%，与参考标准相比准确率80.5%。

Conclusion: 该软件为转移性骨病的监测提供了可靠的工具，适用于临床决策。
...
### [110] [DyGSSM: Multi-view Dynamic Graph Embeddings with State Space Model Gradient Update](https://arxiv.org/abs/2505.09017)
...
Method: 结合GCN提取局部特征和GRU提取全局特征，使用交叉注意力机制整合特征，并基于HiPPO算法的SSM优化参数更新。
...
### [111] [Block-Biased Mamba for Long-Range Sequence Processing](https://arxiv.org/abs/2505.09022)
*Annan Yu,N. Benjamin Erichson*

Main category: cs.LG

TL;DR: Mamba模型在长序列任务中表现不佳，本文通过理论分析提出改进方案B2S6，显著提升性能。


<details>
  <summary>Details</summary>
Motivation: Mamba在长序列任务中的表现与其设计初衷不符，需改进以提高通用性。

Method: 从表达能力、归纳偏置和训练稳定性三方面分析Mamba的不足，并提出B2S6改进方案。

Result: B2S6在长序列任务中优于S4和S4D，同时保持语言建模性能。

Conclusion: B2S6有效解决了Mamba的局限性，提升了其在长序列任务中的表现。
...
### [112] [Single-shot prediction of parametric partial differential equations](https://arxiv.org/abs/2505.09063)
...
<details>
  <summary>Details</summary>
Motivation: 传统方法需要迭代时间步进，效率低下。Flexi-VAE旨在通过神经传播器在变分自编码器（VAE）中直接预测PDEs的解，提高效率和稳定性。

Method: Flexi-VAE结合神经传播器（DCP和PEP）推进潜在表示的时间演化，并通过几何诊断（如雅可比谱分析）优化潜在空间。

Result: 在1D粘性Burgers方程和2D对流-扩散方程上验证，Flexi-VAE在参数范围内实现高精度预测，CPU和GPU速度分别提升50倍和90倍。

Conclusion: Flexi-VAE是一种可扩展且可解释的替代建模工具，适用于计算流体动力学（CFD）等参数PDE驱动的应用，并可扩展到更高维和复杂系统。
...
### [113] [AdaFortiTran: An Adaptive Transformer Model for Robust OFDM Channel Estimation](https://arxiv.org/abs/2505.09076)
*Berkay Guler,Hamid Jafarkhani*

Main category: cs.LG

TL;DR: AdaFortiTran是一种新型深度学习模型，通过结合卷积层和Transformer编码器，显著提升了OFDM系统中快速衰落信道和低信噪比场景下的信道估计性能。
...
Method: 结合卷积层（捕捉局部相关性）和Transformer编码器（全局注意力机制），并整合信道统计信息作为先验，通过残差连接融合全局与局部特征。

Result: 在多种测试条件下，AdaFortiTran的均方误差降低高达6 dB，表现出卓越的鲁棒性。

Conclusion: AdaFortiTran在复杂环境中显著提升了信道估计性能，具有广泛的应用潜力。
...
### [114] [Human-like Cognitive Generalization for Large Models via Brain-in-the-loop Supervision](https://arxiv.org/abs/2505.09085)
*Jiaxuan Chen,Yu Qi,Yueming Wang,Gang Pan*

Main category: cs.LG

TL;DR: 通过结合脑信号监督学习，将人类概念结构迁移到深度神经网络（DNNs），显著提升其对抽象和未知概念的理解能力。


<details>
  <summary>Details</summary>
Motivation: 尽管大规模语言模型在图像和自然语言理解方面表现出色，但实现复杂认知能力（如抽象概念理解、推理和适应新场景）仍是挑战。

Method: 采用脑信号监督学习（brain-in-the-loop supervised learning），利用少量脑信号将人类概念结构迁移到DNNs。

Result: 实验表明，该方法显著提升了DNNs在少样本/零样本学习和分布外识别等任务中的表现，并生成可解释的概念表示。

Conclusion: 人类参与的监督学习能有效增强大模型的复杂认知能力，为开发更接近人类认知的人工系统提供了新途径。
...
### [115] [Generating time-consistent dynamics with discriminator-guided image diffusion models](https://arxiv.org/abs/2505.09089)
*Philipp Hess,Maximilian Gelbrecht,Christof Schötz,Michael Aich,Yu Huang,Shangshang Yang,Niklas Boers*

Main category: cs.LG

TL;DR: 提出了一种时间一致性判别器，使预训练的图像扩散模型能够生成逼真的时空动态，无需额外训练或微调。


<details>
  <summary>Details</summary>
Motivation: 视频扩散模型（VDMs）虽能生成逼真的动态，但训练成本高且资源需求大，限制了广泛应用。

Method: 通过时间一致性判别器引导采样推理过程，无需扩展或微调图像扩散模型。

Result: 在理想湍流模拟和全球降水数据集上表现与从头训练的VDM相当，但具有更好的不确定性校准和更低偏差，支持长期气候模拟。

Conclusion: 该方法为生成逼真时空动态提供了一种高效且资源友好的替代方案。
...
### [116] [Argus: Federated Non-convex Bilevel Learning over 6G Space-Air-Ground Integrated Network](https://arxiv.org/abs/2505.09106)
*Ya Liu,Kai Yang,Yu Zhu,Keying Yang,Haibo Zhao*

Main category: cs.LG

TL;DR: 本文提出了一种名为Argus的异步算法，用于解决SAGIN中的非凸、非光滑分散式联邦双层学习问题，避免了传统同步算法的局限性。


<details>
  <summary>Details</summary>
Motivation: 由于SAGIN的基础设施缺乏和环境时变特性，传统的集中式和同步优化算法不适用，需要开发新的异步算法。

Method: 提出Argus算法，支持网络化代理（如自主飞行器）在时变网络中异步处理双层学习问题。

Result: 提供了Argus的迭代复杂度、通信复杂度和计算复杂度的理论分析，并通过数值实验验证了其有效性。

Conclusion: Argus算法在SAGIN中表现出色，能够有效避免训练速度受拖慢的问题。
...
### [117] [Sequential Treatment Effect Estimation with Unmeasured Confounders](https://arxiv.org/abs/2505.09113)
*Yingrong Wang,Anpeng Wu,Baohong Li,Ziyang Xiao,Ruoxuan Xiong,Qing Han,Kun Kuang*

Main category: cs.LG

TL;DR: 论文提出了一种名为DSIV-CFR的新框架，用于解决序列化治疗中未测量混杂因素的问题，通过分解序列工具变量和负控制假设，显著提升了治疗效果估计的准确性。


<details>
  <summary>Details</summary>
Motivation: 序列化治疗中的未测量混杂因素会影响治疗效果估计，现有方法无法完全解决这一问题，因此需要新的方法来调整潜在混杂偏差。

Method: 提出了DSIV-CFR框架，利用工具变量和负控制假设，通过广义矩条件估计序列治疗效果。

Result: 在4个数据集上的实验表明，该方法在一步和多步预测中表现优异，能够识别动态系统的最优治疗方案。

Conclusion: DSIV-CFR框架有效解决了序列化治疗中的未测量混杂问题，为动态系统的治疗决策提供了可靠工具。
...
### [118] [Fair Clustering via Alignment](https://arxiv.org/abs/2505.09131)
...
<details>
  <summary>Details</summary>
Motivation: 解决现有公平聚类算法因复杂性或近似导致的聚类效用低或数值不稳定的问题。

Method: 基于公平K均值目标函数分解，交替进行数据对齐和聚类中心优化。

Result: FCA在公平性和聚类效用上优于现有方法，且数值稳定。

Conclusion: FCA是一种高效且稳定的公平聚类算法。
...
### [119] [Scaling Gaussian Process Regression with Full Derivative Observations](https://arxiv.org/abs/2505.09134)
*Daniel Huang*
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在处理全导数观测数据时存在扩展性问题，DSoftKI旨在解决这一问题。

Method: 扩展SoftKI的软最大值插值方法，引入方向性信息，构建包含一阶和二阶导数的可扩展近似核。

Result: 在合成函数基准和高维分子力场预测中，DSoftKI表现出高精度，并能处理更大规模数据集。

Conclusion: DSoftKI为处理全导数观测数据提供了一种高效且可扩展的解决方案。
...
### [120] [A Multi-Task Foundation Model for Wireless Channel Representation Using Contrastive and Masked Autoencoder Learning](https://arxiv.org/abs/2505.09160)
*Berkay Guler,Giovanni Geraci,Hamid Jafarkhani*

Main category: cs.LG

TL;DR: 论文提出了WiMAE和ContraWiMAE两种自监督学习方法，专注于无线信道表征，结合了重构和对比学习任务，提升了模型性能和数据效率。


<details>
  <summary>Details</summary>
Motivation: 当前自监督学习在无线信道表征中的应用多借鉴文本和图像处理范式，未充分考虑无线通信的独特特性和约束。

Method: 提出基于Transformer的WiMAE模型，并在其基础上开发ContraWiMAE，结合重构和对比学习任务。

Result: 两种方法在多个下游任务中表现优异，ContraWiMAE在多样无线环境中展现出更强的线性可分性和适应性。

Conclusion: WiMAE和ContraWiMAE为未来无线信道表征学习的自监督研究提供了强大基线。
...
### [121] [Quotient Complex Transformer (QCformer) for Perovskite Data Analysis](https://arxiv.org/abs/2505.09174)
*Xinyu You,Xiang Liu,Chuan-Shen Hu,Kelin Xia,Tze Chien Sum*

Main category: cs.LG

TL;DR: 论文提出了一种基于商复形（QCs）的新表示方法和Quotient Complex Transformer（QCformer）模型，用于预测混合有机-无机钙钛矿（HOIPs）的材料性质，解决了传统图神经网络（GNNs）在周期性结构和高阶相互作用上的不足。


<details>
  <summary>Details</summary>
Motivation: 混合有机-无机钙钛矿（HOIPs）因其优异的光电性能在光伏领域备受关注，但传统GNNs难以捕捉其周期性结构和高阶相互作用，因此需要新的方法。

Method: 提出商复形表示法，通过不同维度的单纯形编码材料结构的成对和多体相互作用，并利用商操作捕捉周期性。设计了基于单纯形的Transformer模块QCformer，先在基准数据集上预训练，再在HOIP数据集上微调。

Result: QCformer在HOIP性质预测上优于现有最先进模型，验证了其有效性。

Conclusion: 商复形表示和QCformer模型为钙钛矿材料的预测建模提供了强大的新工具。
...
### [122] [Optimizing Urban Critical Green Space Development Using Machine Learning](https://arxiv.org/abs/2505.09175)
*Mohammad Ganjirad,Mahmoud Reza Delavar,Hossein Bagheri,Mohammad Mehdi Azizi*

Main category: cs.LG

TL;DR: 本文提出了一种新颖的框架，用于优先发展德黑兰的城市绿地，结合了社会经济、环境和敏感性指标，并通过机器学习模型和微气候模拟验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 德黑兰的城市绿地发展需要科学依据，以优化资源分配并改善城市环境质量。

Method: 利用Google Earth Engine、WRF模型等数据源，结合XGBoost、LightGBM、RF和Extra Trees等机器学习模型进行分类和优先级评估。

Result: RF模型表现最佳，准确率超过94%；绿色屋顶技术可使关键区域气温降低0.67°C。

Conclusion: 该框架为城市规划者提供了科学工具，有效指导绿地发展。
...
### [123] [The Larger the Merrier? Efficient Large AI Model Inference in Wireless Edge Networks](https://arxiv.org/abs/2505.09214)
*Zhonghao Lyu,Ming Xiao,Jie Xu,Mikael Skoglund,Marco Di Renzo*

Main category: cs.LG

TL;DR: 论文提出了一种基于剪枝的大型人工智能模型（LAIM）边缘协同推理方案，通过联合优化剪枝比例、传输功率和计算频率，在资源受限的边缘网络中实现高效推理。


<details>
  <summary>Details</summary>
Motivation: 随着对大型人工智能模型（LAIM）服务的需求增长，传统的云端推理逐渐转向边缘推理以满足低延迟和隐私保护需求。边缘设备与服务器协同推理成为一种资源高效的解决方案。

Method: 论文首先证明了LAIM输出失真受参数失真上界约束，并通过率失真理论推导了参数失真的下界。基于此，提出了一个联合优化问题，并通过高效算法解决这一非凸问题。

Result: 仿真结果表明，参数失真能可靠地约束输出失真，且提出的联合优化设计在推理性能、系统延迟和能耗之间取得了优于基准方案的平衡。

Conclusion: 剪枝比例和资源管理的联合优化在资源受限的边缘环境中显著提升了系统性能，同时证明了分割点在性能优化中的关键作用。
...
### [124] [Birch SGD: A Tree Graph Framework for Local and Asynchronous SGD Methods](https://arxiv.org/abs/2505.09218)
*Alexander Tyurin,Danil Sivtsov*

Main category: cs.LG

TL;DR: Birch SGD是一个新的统一框架，用于分析和设计分布式SGD方法，通过计算树的几何分析简化收敛性分析，并设计了八种新方法，其中六种具有最优计算复杂度。


<details>
  <summary>Details</summary>
Motivation: 为分布式SGD方法提供一个统一的理论基础和分析工具，简化方法设计和性能评估。

Method: 将每种方法表示为加权有向树（计算树），利用树的几何性质分析收敛性，并基于此设计新方法。

Result: 提出了八种新方法，其中六种具有最优计算复杂度；揭示了所有方法的迭代速率相同，但不同方法在性能和通信效率上有不同权衡。

Conclusion: Birch SGD为理解和设计高效异步和并行优化方法提供了统一框架，揭示了方法间的性能权衡。
...
### [125] [Stable and Convexified Information Bottleneck Optimization via Symbolic Continuation and Entropy-Regularized Trajectories](https://arxiv.org/abs/2505.09239)
*Faruk Alpay*
...
<details>
  <summary>Details</summary>
Motivation: 信息瓶颈方法在优化过程中常出现不稳定性，表现为在关键点附近表示突然变化。本文旨在解决这一问题。

Method: 引入熵正则化项，通过符号延续和熵正则化轨迹实现稳定优化，并证明解路径的凸性和唯一性。

Result: 实验结果表明，该方法在广泛的β值范围内稳定了表示学习，并提供了统计鲁棒的不确定性量化。

Conclusion: 本文方法为信息瓶颈的稳定优化提供了理论基础和实用工具，支持未来扩展和实际部署。
...
### [126] [Generating Full-field Evolution of Physical Dynamics from Irregular Sparse Observations](https://arxiv.org/abs/2505.09284)
...
<details>
  <summary>Details</summary>
Motivation: 解决从稀疏和非网格观测中建模和重建多维物理动力学的挑战，现有方法难以处理真实世界物理动力学的连续性和稀疏性。

Method: 利用功能Tucker模型作为潜在空间表示，结合序列扩散模型和时序增强的UNet，通过高斯过程去噪生成核心张量序列。

Result: 在多个物理系统（天文、环境、分子）中验证，SDIFT在重建精度和计算效率上显著优于现有方法。

Conclusion: SDIFT为稀疏观测下的物理动力学建模提供了高效且准确的解决方案，具有广泛的应用潜力。
...
### [127] [On the Learning with Augmented Class via Forests](https://arxiv.org/abs/2505.09294)
*Fan Xu,Wuyang Chen,Wei Gao*

Main category: cs.LG

TL;DR: 论文提出了一种基于增强类别的森林学习方法（LACForest），通过引入增强Gini不纯度作为新的分裂准则，利用测试数据中的未标记数据提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 解决传统决策树和森林方法在测试数据中出现训练数据中未见过的新类别（增强类别）时的性能问题。

Method: 提出增强Gini不纯度作为分裂准则，结合伪标记的增强实例构建浅层森林，并开发基于神经网络的深度森林优化目标。

Result: 实验验证了LACForest方法的有效性，理论分析了增强Gini不纯度的收敛性。

Conclusion: LACForest通过结合增强类别信息和神经网络表示能力，提升了森林模型在测试数据中的泛化性能。
...
### [128] [Neural Multivariate Regression: Qualitative Insights from the Unconstrained Feature Model](https://arxiv.org/abs/2505.09308)
*George Andriopoulos,Soyuj Jung Basnet,Juan Guevara,Li Guo,Keith Ross*

Main category: cs.LG

TL;DR: 论文利用无约束特征模型（UFM）分析深度神经网络中的多任务模型与单任务模型的训练性能差异，以及目标归一化对训练的影响。


<details>
  <summary>Details</summary>
Motivation: 研究多任务模型与单任务模型的性能对比，以及目标归一化对训练效果的影响，为深度神经网络设计和数据预处理提供理论支持。

Method: 基于UFM理论进行数学建模，并通过实验验证理论预测。

Result: 多任务模型在相同或更强正则化下训练误差更小；目标归一化在目标维度平均方差小于1时能降低训练误差。

Conclusion: UFM是分析DNN设计和数据预处理策略的强大框架。
...
### [129] [MUST: Multi-Scale Structural-Temporal Link Prediction Model for UAV Ad Hoc Networks](https://arxiv.org/abs/2505.09331)
*Cunlai Pu,Fangrui Wu,Rajput Ramiz Sharafat,Guangzhao Dai,Xiangbo Shu*

Main category: cs.LG

TL;DR: 论文提出了一种多尺度结构-时间链接预测模型（MUST），用于无人机自组网（UANETs）中的链接预测，解决了动态和稀疏拓扑带来的挑战。


<details>
  <summary>Details</summary>
Motivation: 在对抗环境中，无人机自组网的链接预测仅能依赖历史拓扑信息，而动态和稀疏性使得传统方法难以捕捉有效模式。

Method: 结合图注意力网络（GATs）捕捉多尺度结构特征，并使用LSTM学习时间动态，同时通过优化损失函数解决稀疏性问题。

Result: 实验表明，MUST在动态和稀疏的UANETs中实现了最先进的链接预测性能。

Conclusion: MUST模型在多尺度结构-时间特征捕捉和稀疏性处理方面表现优异，适用于高度动态的UANETs。
...
### [130] [GreenFactory: Ensembling Zero-Cost Proxies to Estimate Performance of Neural Networks](https://arxiv.org/abs/2505.09344)
*Gabriel Cortês,Nuno Lourenço,Paolo Romano,Penousal Machado*

Main category: cs.LG

TL;DR: GreenFactory是一种集成零成本代理的方法，通过随机森林回归器直接预测模型测试准确率，解决了传统代理方法泛化能力不足的问题。
...
### [131] [Exploiting the Potential Supervision Information of Clean Samples in Partial Label Learning](https://arxiv.org/abs/2505.09354)
*Guangtai Wang,Chi-Man Vong,Jintao Huang*

Main category: cs.LG

TL;DR: 论文提出了一种名为CleanSE的校准策略，通过利用干净样本增强部分标签学习中的候选标签置信度，结合可微分计数损失和K近邻算法，显著提升了现有方法的性能。


<details>
  <summary>Details</summary>
Motivation: 减少假阳性标签的影响对部分标签学习中的消歧至关重要。现有方法主要关注单个部分标签实例的特性，而忽略了数据集中随机存在的干净样本的强监督信息。

Method: 提出CleanSE策略，利用干净样本为候选标签提供指导，并通过可微分计数损失和K近邻算法校准候选标签的置信度。

Result: 在3个合成基准和5个真实PLL数据集上的实验表明，CleanSE能显著提升现有PLL方法的性能。

Conclusion: CleanSE策略通过利用干净样本增强候选标签置信度，为部分标签学习提供了一种有效的校准方法。
...
### [132] [Efficient Mixed Precision Quantization in Graph Neural Networks](https://arxiv.org/abs/2505.09361)
...
Method: 提出定理保证整数消息传递的数值等价性，并开发MixQ-GNN框架灵活选择位宽组合。

Result: MixQ-GNN平均减少5.5x（节点分类）和5.1x（图分类）的位操作。

Conclusion: MixQ-GNN高效且性能接近FP32精度，适用于大规模图应用。
...
### [133] [Personalized Control for Lower Limb Prosthesis Using Kolmogorov-Arnold Networks](https://arxiv.org/abs/2505.09366)
*SeyedMojtaba Mohasel,Alireza Afzal Aghaei,Corey Pew*

Main category: cs.LG

TL;DR: 研究了可学习激活函数在KANs中用于下肢假肢个性化控制的潜力，并比较了用户特定数据与池化数据对ML/DL性能的影响。


<details>
  <summary>Details</summary>
Motivation: 探索可学习激活函数在KANs中的表现，以及训练数据来源对模型性能的影响，以优化假肢控制。

Method: 收集IMU数据，比较MLP、KAN、CNN和FKAN在用户特定和池化数据上的表现。

Result: 可学习激活函数未显著提升性能；用户特定数据对ML模型更有效，而DL模型对数据来源不敏感。

Conclusion: 可学习激活函数可能在更复杂任务中表现更好；DL模型可利用多用户数据训练。
...
### [134] [SafePath: Conformal Prediction for Safe LLM-Based Autonomous Navigation](https://arxiv.org/abs/2505.09427)
*Achref Doula,Max Mühläuser,Alejandro Sanchez Guinea*

Main category: cs.LG

TL;DR: SafePath是一个模块化框架，通过整合LLM和共形预测，为自动驾驶路径规划提供形式化安全保证，显著降低碰撞率和不确定性。
...
Method: SafePath分三阶段：1）LLM生成多样化候选路径；2）通过共形预测过滤高风险路径；3）根据不确定性选择最优路径或人工接管。

Result: 实验表明，SafePath减少77%的不确定性和70%的碰撞率，显著提升安全性。

Conclusion: SafePath通过形式化安全保证和灵活的人工介入机制，有效平衡了LLM路径规划的自主性与安全性。
...
### [135] [Establishing Linear Surrogate Regret Bounds for Convex Smooth Losses via Convolutional Fenche-Young Losses](https://arxiv.org/abs/2505.09432)
*Yuzhou Cao,Han Bao,Lei Feng,Bo An*

Main category: cs.LG

TL;DR: 论文提出了一种克服凸平滑替代损失与线性后悔界之间权衡的方法，通过构造基于卷积负熵的Fenchel-Young损失，实现了线性后悔界并保持了平滑性。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于解决凸平滑替代损失在优化和估计上的优势与线性后悔界之间的权衡问题。

Method: 方法是通过构造基于卷积负熵的Fenchel-Young损失，结合定制的预测链接，实现线性后悔界并保持平滑性。

Result: 结果表明，该方法不仅保持了替代损失的平滑性，还实现了线性后悔界，并提供了对底层类别概率的一致估计。

Conclusion: 结论是该方法展示了凸分析在风险最小化中的优化和统计效率上的新颖应用。
...
### [136] [CXMArena: Unified Dataset to benchmark performance in realistic CXM Scenarios](https://arxiv.org/abs/2505.09436)
*Raghav Garg,Kapil Sharma,Karan Gupta*

Main category: cs.LG

TL;DR: 论文介绍了CXMArena，一个用于评估AI在客户体验管理（CXM）中实用性的合成基准数据集，填补了现有基准在真实性和任务多样性上的不足。


<details>
  <summary>Details</summary>
Motivation: 由于数据稀缺和现有基准的局限性（缺乏真实性和深度知识库集成），难以评估大型语言模型（LLMs）在复杂客户体验管理环境中的实际效用。

Method: 开发了一个基于LLM的可扩展管道，模拟品牌CXM实体（如知识文章、问题分类和对话），并通过噪声注入和自动化验证确保数据真实性。

Result: CXMArena包含五个关键任务的基准测试，实验显示当前最先进的模型在任务中表现不佳（如文章搜索准确率68%），凸显了现有技术的挑战。

Conclusion: CXMArena为评估AI在CXM中的实用性提供了更真实的基准，揭示了当前模型在复杂任务中的局限性，需开发更先进的解决方案。
...
### [137] [Variational Rank Reduction Autoencoder](https://arxiv.org/abs/2505.09458)
*Jad Mounayer,Alicia Tierz,Jerome Tomezyk,Chady Ghnatios,Francisco Chinesta*

Main category: cs.LG

TL;DR: VRRAEs结合RRAEs和VAEs的优点，通过SVD正则化和KL散度优化，在生成任务中表现优于两者，并减少后验崩溃风险。


<details>
  <summary>Details</summary>
Motivation: 结合确定性RRAEs和概率性VAEs的优势，提升生成模型性能并解决后验崩溃问题。

Method: 提出VRRAEs模型，利用SVD正则化和KL散度优化，对RRAEs的潜在空间进行采样。

Result: 在合成数据集和MNIST、CelebA、CIFAR-10上，VRRAEs在生成和插值任务中表现优于RRAEs和VAEs。

Conclusion: VRRAEs通过结合SVD正则化和KL散度，显著提升了生成能力并减少了后验崩溃的可能性。
...
### [138] [Preserving Plasticity in Continual Learning with Adaptive Linearity Injection](https://arxiv.org/abs/2505.09486)
*Seyed Roozbeh Razavi Rohani,Khashayar Khajavi,Wesley Chung,Mo Chen,Sharan Vaswani*

Main category: cs.LG

TL;DR: 论文提出了一种名为AdaLin的方法，通过动态调整神经元的激活函数来缓解深度神经网络中的可塑性损失问题。该方法无需额外超参数或任务边界，显著提升了多个基准任务的性能。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络在非稳态问题中逐渐丧失增量学习能力（可塑性损失），而深度线性网络对此具有较强抵抗力。受此启发，研究旨在通过自适应线性化解决这一问题。

Method: AdaLin为每个神经元引入可学习参数和门控机制，根据梯度流动态调整激活函数的线性化程度，从而维持梯度信号和持续学习能力。

Result: 在多个基准测试（如MNIST、CIFAR-10、CIFAR-100）和复杂场景（如类增量学习、强化学习）中，AdaLin显著提升了性能，并通过消融实验验证了神经元级适应的重要性。

Conclusion: AdaLin通过动态线性化有效缓解了可塑性损失，为持续学习提供了一种通用且高效的解决方案。
...
### [139] [Layered Unlearning for Adversarial Relearning](https://arxiv.org/abs/2505.09500)
*Timothy Qian,Vinith Suriyakumar,Ashia Wilson,Dylan Hadfield-Menell*

Main category: cs.LG

TL;DR: 研究后训练方法（如微调、对齐和遗忘）如何改变语言模型行为及表征，并提出分层遗忘（LU）算法以提高对抗性再学习的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 探究后训练方法修改语言模型行为的脆弱性及其易被提示工程或再学习绕过的原因。

Method: 设计分层遗忘（LU）算法，通过分阶段遗忘数据子集来抑制再学习恢复完整数据的能力。

Result: LU算法提高了多种遗忘方法对抗再学习的鲁棒性。

Conclusion: LU算法为机器遗忘领域提供新方法，并揭示了后训练更新的影响。
...
### [140] [Towards Fair In-Context Learning with Tabular Foundation Models](https://arxiv.org/abs/2505.09503)
*Patrik Kenfack,Samira Ebrahimi Kaho,Ulrich Aïvodji*

Main category: cs.LG

TL;DR: 论文研究了表格基础模型在上下文学习（ICL）中的公平性问题，并提出了三种预处理策略来减少偏见，其中基于不确定性的演示选择效果最佳。


<details>
  <summary>Details</summary>
Motivation: 尽管表格基础模型在上下文学习中表现出色，但其公平性尚未明确，因此需要研究偏见的表现形式及解决方法。

Method: 采用三种预处理策略：相关性去除、组平衡演示选择和基于不确定性的演示选择，以评估其对公平性的影响。

Result: 实验表明，基于不确定性的演示选择能显著提升上下文预测的组公平性。
...
### [141] [SAD Neural Networks: Divergent Gradient Flows and Asymptotic Optimality via o-minimal Structures](https://arxiv.org/abs/2505.09572)
*Julian Kranz,Davide Gallon,Steffen Dereich,Arnulf Jentzen*

Main category: cs.LG

TL;DR: 研究全连接前馈神经网络梯度流的收敛性，证明梯度流要么收敛到临界点，要么发散到无穷大，且损失收敛到渐近临界值。


<details>
  <summary>Details</summary>
Motivation: 探索神经网络梯度流的动态行为，特别是在常见激活函数下的收敛性和发散性。

Method: 利用o-minimal结构的几何性质，结合理论证明和数值实验验证。

Result: 证明存在阈值ε，初始化损失在ε内的梯度流会收敛到最优损失值；对于多项式目标函数，最优损失值为零且只能渐近实现。

Conclusion: 梯度流在足够好的初始化下会发散到无穷大，理论结果在真实场景中也有类似行为。
...
### [142] [Rhomboid Tiling for Geometric Graph Deep Learning](https://arxiv.org/abs/2505.09586)
*Yipeng Zhang,Longlong Li,Kelin Xia*

Main category: cs.LG

TL;DR: 论文提出了一种基于菱形平铺结构（Rhomboid Tiling, RT）的聚类方法，并设计了RTPool模型，用于图分类任务，显著优于现有方法。
...
Method: 提出RT聚类方法，利用数据的复杂几何信息进行聚类；设计RTPool模型，结合RT聚类实现层次化图聚类。

Result: RTPool在7个基准数据集上优于21种现有方法。
...
### [143] [Online Isolation Forest](https://arxiv.org/abs/2505.09593)
...
<details>
  <summary>Details</summary>
Motivation: 现有离线异常检测方法在流数据场景下不切实际，而在线方法通常需要定期重新训练，无法满足实时需求。

Method: 提出Online-iForest，一种专为流数据设计的方法，能够动态跟踪数据生成过程的变化。

Result: 实验表明，Online-iForest性能与在线方法相当，接近离线方法，且在效率上显著优于其他方法。

Conclusion: Online-iForest是一种高效的在线异常检测方法，适用于需要快速识别异常的应用场景。
...
### [144] [Adversarial Suffix Filtering: a Defense Pipeline for LLMs](https://arxiv.org/abs/2505.09602)
*David Khachaturov,Robert Mullins*

Main category: cs.LG

TL;DR: 论文提出了一种轻量级、模型无关的防御方法ASF，用于保护大语言模型免受对抗性后缀攻击，显著降低攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在自主系统和公共环境中的应用日益广泛，但其易受越狱漏洞影响，现有防御方法存在局限性。

Method: 提出Adversarial Suffix Filtering (ASF)，作为输入预处理和净化器，检测并过滤对抗性后缀。

Result: ASF在黑白盒攻击场景下均有效，将攻击成功率降至4%以下，且对模型正常能力影响极小。

Conclusion: ASF是一种高效、通用的防御方案，适用于多种部署环境。
...
### [145] [A Comparative Study of Transformer-Based Models for Multi-Horizon Blood Glucose Prediction](https://arxiv.org/abs/2505.08821)
...
<details>
  <summary>Details</summary>
Motivation: 通过精准的血糖预测，可以为1型糖尿病治疗提供个性化干预措施（如胰岛素和饮食调整），但Transformer模型在此领域的潜力尚未充分探索。

Method: 使用公开数据集DCLP3和OhioT1DM，比较了点状、块状、序列状和混合嵌入的Transformer模型，评估了不同时间跨度的预测效果。

Result: Crossformer在30分钟预测中表现最佳（RMSE 15.6 mg/dL），PatchTST在长期预测（1h、2h、4h）中表现最优（RMSE 24.6-46.5 mg/dL）。

Conclusion: 基于块的Transformer模型能有效捕捉多变量时间序列的季节性模式，显著提升血糖预测的准确性。
...
### [146] [CellTypeAgent: Trustworthy cell type annotation with Large Language Models](https://arxiv.org/abs/2505.08844)
*Jiawen Chen,Jianghao Zhang,Huaxiu Yao,Yun Li*

Main category: q-bio.GN

TL;DR: CellTypeAgent是一个基于大型语言模型（LLM）的代理工具，用于单细胞RNA测序分析中的细胞类型注释，通过结合数据库验证提高了准确性并减少了幻觉问题。
...
Method: 结合大型语言模型（LLM）和相关数据库验证，开发了CellTypeAgent工具。
...
### [147] [When repeats drive the vocabulary: a Byte-Pair Encoding analysis of T2T primate genomes](https://arxiv.org/abs/2505.08918)
*Marina Popova,Iaroslav Chelombitko,Aleksey Komissarov*

Main category: q-bio.GN

TL;DR: 研究探索了Byte Pair Encoding（BPE）在T2T灵长类基因组中的应用，发现共享词汇量随基因组比较增加而快速下降，且BPE对高拷贝重复序列敏感，限制了其在比较基因组学中的通用性。
...
Method: 研究使用自定义工具dnaBPE，对9个T2T灵长类基因组（包括3个人类组装）应用BPE标记化，固定词汇量为512,000个标记。

Result: 分析显示仅11,569个标记在所有组装中共享，而991,854个标记为单一基因组独有；基于标记重叠的系统发育树未能重现已知灵长类关系。
...
### [148] [TensorRL-QAS: Reinforcement learning with tensor networks for scalable quantum architecture search](https://arxiv.org/abs/2505.09371)
*Akash Kundu,Stefano Mangini*

Main category: quant-ph

TL;DR: TensorRL-QAS结合张量网络和强化学习，提出了一种可扩展的量子电路设计框架，显著提升了效率和性能。


<details>
  <summary>Details</summary>
Motivation: 解决基于强化学习的量子架构搜索（QAS）在可扩展性和计算成本上的挑战。

Method: 结合张量网络（TN）和强化学习（RL），通过矩阵乘积状态近似目标解来缩小搜索空间。

Result: 在12量子比特的量子化学问题中，CNOT门数量和电路深度减少10倍，训练速度提升98%，成功概率达50%。
...
### [149] [Quantum-Enhanced Parameter-Efficient Learning for Typhoon Trajectory Forecasting](https://arxiv.org/abs/2505.09395)
*Chen-Yu Liu,Kuan-Cheng Chen,Yi-Chien Chen,Samuel Yen-Chi Chen,Wei-Hao Huang,Wei-Jia Huang,Yen-Jui Chang*

Main category: quant-ph

TL;DR: QT框架结合量子神经网络（QNNs）和经典方法，提出Quantum Parameter Adaptation（QPA）用于台风轨迹预测，减少可训练参数并保持性能。


<details>
  <summary>Details</summary>
Motivation: 台风轨迹预测对灾害准备至关重要，但传统方法计算资源需求高，量子机器学习（QML）提供了一种高效解决方案。

Method: 提出QPA方法，结合Attention-based Multi-ConvGRU模型，利用量子神经网络生成训练参数，推理时无需量子硬件。

Result: QPA显著减少可训练参数数量，同时保持预测准确性，为气候建模提供可扩展且节能的方法。

Conclusion: QT框架首次应用于大规模台风轨迹预测，展示了混合量子-经典学习在高效和可持续预测中的潜力。
...
### [150] [Quantum state-agnostic work extraction (almost) without dissipation](https://arxiv.org/abs/2505.09456)
*Josep Lumbreras,Ruo Cheng Huang,Yanglin Hu,Mile Gu,Marco Tomamichel*

Main category: quant-ph

TL;DR: 论文研究了通过顺序访问未知纯量子比特状态来最大化提取能量到电池的协议，利用强化学习中的探索-开发权衡实现能量耗散的优化。


<details>
  <summary>Details</summary>
Motivation: 设计协议以平衡两个目标：利用当前量子比特最优充电，同时获取更多信息以提升后续能量提取效率。

Method: 采用强化学习中的探索-开发权衡策略，开发自适应方法。

Result: 能量耗散仅随N的多对数增长，相比基于全态层析的现有协议有指数级改进。

Conclusion: 自适应策略显著提升了能量提取效率，为量子能量提取提供了新思路。
...
### [151] [WorldView-Bench: A Benchmark for Evaluating Global Cultural Perspectives in Large Language Models](https://arxiv.org/abs/2505.09595)
*Abdullah Mushtaq,Imran Taj,Rafay Naeem,Ibrahim Ghaznavi,Junaid Qadir*

Main category: cs.CL

TL;DR: 论文提出WorldView-Bench基准，评估大语言模型（LLMs）的全球文化包容性（GCI），通过多视角干预策略显著提升文化多样性表现。
...
Method: 引入WorldView-Bench基准，基于Multiplex Worldview理论，采用自由生成评估和两种干预策略（上下文实现和多代理系统实现）。

Result: 多代理系统干预使视角分布熵从13%提升至94%，情感倾向转为67.7%正面，文化平衡增强。
...
### [152] [Human-AI Collaboration or Academic Misconduct? Measuring AI Use in Student Writing Through Stylometric Evidence](https://arxiv.org/abs/2505.08828)
*Eduardo Araujo Oliveira,Madhavi Mohoni,Sonsoles López-Pernas,Mohammed Saqr*

Main category: cs.CL

TL;DR: 研究利用作者验证技术量化AI在学术写作中的辅助作用，促进透明度和学生发展。


<details>
  <summary>Details</summary>
Motivation: 随着人机协作在教育中的普及，理解和测量这种互动的程度与性质成为挑战。

Method: 研究分为三个阶段：数据集选择与扩展、AV方法开发、系统评估，使用三个数据集和540个作者问题。

Result: 改进的AV分类器能识别风格差异，测量人机协作，并为教育者提供透明工具。

Conclusion: 研究推进了AV技术，为AI时代的学术写作动态提供了实用见解。
...
### [153] [CEC-Zero: Chinese Error Correction Solution Based on LLM](https://arxiv.org/abs/2505.09082)
*Sophie Zhang,Zhiming Lin*

Main category: cs.CL

TL;DR: CEC-Zero是一种基于强化学习的框架，通过自主错误策略学习提升大语言模型在中文拼写纠正中的表现，无需外部监督。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在中文文本处理（如拼写纠正）中表现出色，但其可靠性和泛化能力仍有不足。

Method: 结合强化学习与大语言模型的生成能力，提出CEC-Zero框架，实现自主错误策略学习。

Result: 实验显示，强化学习增强的大语言模型在准确性和跨领域泛化能力上达到行业可用水平。
...
### [154] [Multilingual Machine Translation with Quantum Encoder Decoder Attention-based Convolutional Variational Circuits](https://arxiv.org/abs/2505.09407)
*Subrit Dikshit,Ritu Tiwari,Priyank Jain*

Main category: cs.CL

TL;DR: QEDACVC是一种基于量子计算的编码器-解码器架构，用于多语言机器翻译，在OPUS数据集上达到82%的准确率。


<details>
  <summary>Details</summary>
Motivation: 探索量子计算在多语言翻译中的应用，以替代传统的经典计算方法。
...
### [155] [In-Context Learning for Label-Efficient Cancer Image Classification in Oncology](https://arxiv.org/abs/2505.08798)
*Mobina Shrestha,Bishwas Mandal,Vishal Mandal,Asis Shrestha*

Main category: eess.IV

TL;DR: 研究探讨了上下文学习（ICL）作为模型微调替代方案在肿瘤学中的应用，通过少量标注样本实现任务适应，并在多个视觉语言模型上验证其性能。


<details>
  <summary>Details</summary>
Motivation: AI在肿瘤学中的应用受限于对大量标注数据和模型微调的依赖，研究旨在探索ICL的实用性，以解决资源受限环境下的诊断任务。

Method: 使用四种视觉语言模型（Paligemma、CLIP、ALIGN、GPT-4o）在三个肿瘤学数据集（MHIST、PatchCamelyon、HAM10000）上评估ICL性能。

Result: 所有模型在少量样本提示下表现显著提升，GPT-4o在二元和多类分类中分别达到F1分数0.81和0.60。开源模型如Paligemma和CLIP也表现竞争力。

Conclusion: ICL在肿瘤学中具有潜力，尤其适用于罕见癌症和资源受限环境，为无法微调和标注数据稀缺的场景提供实用解决方案。
...
### [156] [Ultrasound Report Generation with Multimodal Large Language Models for Standardized Texts](https://arxiv.org/abs/2505.08838)
*Peixuan Ge,Tongkun Su,Faqin Lv,Baoliang Zhao,Peng Zhang,Chi Hong Wong,Liang Yao,Yu Sun,Zenan Wang,Pak Kin Wong,Ying Hu*

Main category: eess.IV

TL;DR: 提出了一种统一的多器官和多语言超声报告生成框架，通过片段化多语言训练和标准化报告特性，显著提升了生成报告的准确性和一致性。


<details>
  <summary>Details</summary>
Motivation: 超声图像变异性大、依赖操作者且缺乏标准化数据集，导致自动化报告生成困难。

Method: 整合片段化多语言训练，利用标准化报告特性，通过双语数据集和选择性解冻视觉变换器（ViT）优化文本-图像对齐。

Result: 相比KMVE方法，BLEU提升2%，ROUGE-L提升3%，CIDEr提升15%，显著减少错误内容。

Conclusion: 该框架在多器官和多语言场景下具有临床实用潜力。
...
### [157] [Validation of Conformal Prediction in Cervical Atypia Classification](https://arxiv.org/abs/2505.08845)
...
TL;DR: 本文探讨了使用共形预测框架改进深度学习模型在宫颈癌分类中的不确定性表达，并通过专家标注验证其真实性和实用性。
...
Method: 研究评估了三种共形预测方法在三种深度学习模型上的表现，使用多标注者的专家标注集进行验证，并分析了预测集与人类标注的一致性。

Result: 传统基于覆盖率的评估高估了性能，当前共形预测方法生成的预测集与人类标注不一致，且在识别模糊和分布外数据方面表现有限。

Conclusion: 共形预测需进一步优化以提高预测集的真实性和实用性，确保其与人类预期一致。
...
### [158] [Thoughts on Objectives of Sparse and Hierarchical Masked Image Model](https://arxiv.org/abs/2505.08819)
*Asahi Miyazaki,Tsuyoshi Okita*

Main category: eess.IV

TL;DR: 论文提出了一种新的掩码模式Mesh Mask-ed SparK，用于改进SparK模型的性能，并研究了掩码模式对预训练效果的影响。


<details>
  <summary>Details</summary>
Motivation: 探索掩码模式对自监督学习模型性能的影响，尤其是在SparK模型中。

Method: 提出Mesh Mask-ed SparK模型，采用新的掩码模式进行图像掩码预训练。

Result: 研究发现掩码模式对模型性能有显著影响。

Conclusion: Mesh Mask-ed SparK模型通过改进掩码模式，提升了SparK模型的性能。
...
### [159] [Total Variation-Based Image Decomposition and Denoising for Microscopy Images](https://arxiv.org/abs/2505.08843)
...
<details>
  <summary>Details</summary>
Motivation: 显微镜图像常受噪声干扰，影响质量与特征提取，需要现代去噪与恢复方案。

Method: 通过提取并去除图像中的噪声成分或直接去噪，评估TV-L1、Huber-ROF和TGV-L1的性能。

Result: Huber-ROF表现最灵活，TGV-L1最适合去噪，方法适用于多种显微镜技术。

Conclusion: 该方法在显微镜领域有广泛应用潜力，代码已公开便于集成或独立使用。
...
### [160] [BiECVC: Gated Diversification of Bidirectional Contexts for Learned Video Compression](https://arxiv.org/abs/2505.09193)
*Wei Jiang,Junru Li,Kai Zhang,Li Zhang*

Main category: eess.IV

TL;DR: BiECVC是一种双向视频压缩框架，通过多样化上下文建模和自适应门控机制，显著提升了性能，首次在RA配置下超越VTM 13.2。
...
Method: BiECVC结合局部和非局部上下文建模，重用高质量特征并采用线性注意力机制，同时引入双向上下文门控动态过滤信息。

Result: 实验显示BiECVC在RA配置下比特率降低13.4%和15.7%，首次在所有标准测试数据集上超越VTM 13.2。
...
### [161] [Q-space Guided Collaborative Attention Translation Network for Flexible Diffusion-Weighted Images Synthesis](https://arxiv.org/abs/2505.09323)
...
<details>
  <summary>Details</summary>
Motivation: 解决现有方法在灵活q空间采样下合成DWI数据的局限性，同时保持解剖学保真度。

Method: 采用协作注意力机制，结合多模态信息，并引入任务特定约束以保持DWI的解剖学保真度。

Result: 在HCP数据集上，Q-CATN在参数图和纤维束估计上优于现有方法，且能适应灵活的q空间采样。

Conclusion: Q-CATN是一种有潜力的工具，适用于临床和研究应用，代码已开源。
...
### [162] [DCSNet: A Lightweight Knowledge Distillation-Based Model with Explainable AI for Lung Cancer Diagnosis from Histopathological Images](https://arxiv.org/abs/2505.09334)
*Sadman Sakib Alif,Nasim Anzum Promise,Fiaz Al Abid,Aniqua Nusrat Zereen*

Main category: eess.IV

TL;DR: 论文提出了一种基于知识蒸馏和可解释AI的轻量级模型（DCSNet），用于肺癌检测，解决了深度学习模型在资源受限环境中的计算成本和透明度问题。


<details>
  <summary>Details</summary>
Motivation: 肺癌是全球癌症相关死亡的主要原因，早期检测和准确诊断对提高生存率至关重要。深度学习模型虽然有效，但计算成本高且缺乏透明度，限制了其在医疗领域的应用。

Method: 采用知识蒸馏技术，将复杂教师模型（如ResNet50）的知识转移到轻量级学生模型（DCSNet），并结合可解释AI技术提升透明度。评估了八种CNN作为教师模型。

Result: 提出的DCSNet在资源受限环境中表现出高诊断性能，同时通过可解释AI增强了模型透明度。

Conclusion: 该方法为医疗领域提供了一种高效、透明的AI诊断工具，有望推动深度学习在肺癌检测中的广泛应用。
...
### [163] [Spec2VolCAMU-Net: A Spectrogram-to-Volume Model for EEG-to-fMRI Reconstruction based on Multi-directional Time-Frequency Convolutional Attention Encoder and Vision-Mamba U-Net](https://arxiv.org/abs/2505.09521)
*Dongyi He,Shiyang Li,Bin Jiang,He Yan*

Main category: eess.IV

TL;DR: Spec2VolCAMU-Net是一种轻量级的频谱图到体积生成器，通过多方向时间-频率卷积注意力编码器和Vision-Mamba U-Net解码器，显著提升了EEG到fMRI的生成质量，并在多个基准测试中取得了最佳性能。


<details>
  <summary>Details</summary>
Motivation: 高分辨率fMRI成本高且操作复杂，而EEG更易获取。现有EEG-to-fMRI生成器存在性能或效率问题，因此需要一种更高效的解决方案。

Method: 提出Spec2VolCAMU-Net，结合多方向时间-频率卷积注意力编码器和Vision-Mamba U-Net解码器，采用混合SSI-MSE损失进行端到端训练。

Result: 在三个公开基准测试中，SSIM分别提升14.5%、14.9%和16.9%，PSNR在CN-EPFL数据集上提升4.6%。

Conclusion: Spec2VolCAMU-Net在重建质量和效率上表现优异，适合临床和研究中的实时应用。
...
### [164] [Meta-learning Slice-to-Volume Reconstruction in Fetal Brain MRI using Implicit Neural Representations](https://arxiv.org/abs/2505.09565)
...
TL;DR: 提出了一种基于隐式神经表示的高分辨率切片到体积重建（SVR）方法，用于处理运动伪影严重的MRI图像，显著提升了重建质量和速度。
...
Result: 在模拟和临床MRI数据上验证了方法的有效性，重建质量优于现有技术，重建时间减少达50%。
...
### [165] [ELIS: Efficient LLM Iterative Scheduling System with Response Length Predictor](https://arxiv.org/abs/2505.09142)
*Seungbeom Choi,Jeonghoe Goo,Eunjoo Jeon,Mingyu Yang,Minsung Jang*

Main category: cs.DC

TL;DR: ELIS是一个用于大型语言模型（LLM）的服务系统，采用迭代最短剩余时间优先（ISRTF）调度器，通过预测剩余令牌数优化任务调度，解决了传统FCFS策略的“队头阻塞”问题。实验显示，ISRTF可将平均任务完成时间降低19.6%。


<details>
  <summary>Details</summary>
Motivation: 当前LLM服务系统采用先到先服务（FCFS）调度策略，易导致“队头阻塞”问题，需通过预测推理时间并应用最短作业优先策略来优化。

Method: ELIS通过BGE模型训练LLM响应长度预测器，并设计ISRTF调度策略，结合迭代批处理优化调度。实验基于真实用户请求模拟和Kubernetes实现。

Result: ISRTF调度策略显著提升性能，平均任务完成时间减少达19.6%。

Conclusion: ELIS通过ISRTF调度器和响应长度预测，有效优化LLM服务系统的任务调度，显著降低延迟。
...
### [166] [Insights into DeepSeek-V3: Scaling Challenges and Reflections on Hardware for AI Architectures](https://arxiv.org/abs/2505.09343)
...
TL;DR: DeepSeek-V3通过硬件与模型协同设计，解决了大规模语言模型在内存、计算效率和互联带宽上的限制，提出了多项创新技术，并探讨了未来硬件发展方向。


<details>
  <summary>Details</summary>
Motivation: 当前硬件架构在大规模语言模型训练中存在内存容量、计算效率和互联带宽的限制，亟需解决方案。

Method: 采用Multi-head Latent Attention、Mixture of Experts架构、FP8混合精度训练和Multi-Plane网络拓扑等技术。

Result: DeepSeek-V3实现了高效的大规模训练和推理，并提供了硬件与模型协同设计的实践蓝图。

Conclusion: 硬件与模型协同设计是满足AI工作负载需求的关键，为下一代AI系统创新提供了方向。
...
### [167] [WavReward: Spoken Dialogue Models With Generalist Reward Evaluators](https://arxiv.org/abs/2505.09558)
...
TL;DR: 论文提出了WavReward，一种基于音频语言模型的奖励反馈模型，用于评估语音对话系统的IQ和EQ表现，并在多个场景中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有语音对话模型的评估主要依赖文本模型，忽略了非文本信息的复杂性，因此需要一种专门针对语音对话的评估方法。

Method: 基于音频语言模型，WavReward结合深度推理和非线性奖励机制，通过强化学习算法构建评估器，并使用ChatReward-30K数据集进行训练。

Result: WavReward在客观准确率上从55.1%提升至91.5%，主观A/B测试中领先83%，验证了其有效性。

Conclusion: WavReward为语音对话系统提供了更全面的评估工具，其组件设计均被证明是必要的。
...
### [168] [Lower Bounds on the MMSE of Adversarially Inferring Sensitive Features](https://arxiv.org/abs/2505.09004)
...
<details>
  <summary>Details</summary>
Motivation: 研究如何在有限样本和线性模型下评估敏感特征推断的对抗性，确保理论保证与实际效率的平衡。

Method: 采用MMSE估计，推导了线性模型下敏感特征推断的闭式下界，并引入验证集计算新下界。

Result: 推导了噪声方差最优的闭式下界，并通过实验验证了框架的有效性。
...
### [169] [Risk Bounds For Distributional Regression](https://arxiv.org/abs/2505.09075)
*Carlos Misael Madrid Padilla,Oscar Hernan Madrid Padilla,Sabyasachi Chatterjee*

Main category: stat.ML

TL;DR: 本文研究了非参数分布回归估计器的风险界限，提出了凸约束和非凸约束下的理论界限，并通过实验验证了其有效性。
...
Method: 建立凸约束和非凸约束下的分布回归模型，推导CRPS和MSE的理论界限。

Result: 理论界限与均值估计的收敛速率一致，实验验证了其实际效果。

Conclusion: 提出的理论界限适用于多种分布回归模型，具有实际应用价值。
...
### [170] [Online Learning of Neural Networks](https://arxiv.org/abs/2505.09167)
*Amit Daniely,Idan Mehalel,Elchanan Mossel*

Main category: stat.ML

TL;DR: 研究了带有符号激活函数的前馈神经网络在线学习问题，分析了边缘条件对学习能力的影响，并提出了两种自然限制以降低维度依赖性。


<details>
  <summary>Details</summary>
Motivation: 探讨神经网络在线学习的边界条件，特别是在高维情况下如何减少错误率。

Method: 通过分析边缘条件和构造特定网络，证明了错误边界与维度d的关系，并提出了多索引模型和扩展边缘假设两种限制。

Result: 证明了最优错误边界约为TS(d,γ)，并展示了在高维情况下错误率可能指数增长；在多索引模型和扩展边缘假设下，错误边界显著降低。

Conclusion: 边缘条件对神经网络在线学习至关重要，通过引入额外限制可以有效降低维度依赖性，提高学习效率。
...
### [171] [Optimal Transport-Based Domain Adaptation for Rotated Linear Regression](https://arxiv.org/abs/2505.09229)
*Brian Britos,Mathias Bourel*

Main category: stat.ML

TL;DR: 论文提出了一种基于最优传输（OT）的监督域适应方法，用于解决线性回归模型在旋转偏移下的问题，并通过K-means聚类、OT和SVD估计旋转角度。


<details>
  <summary>Details</summary>
Motivation: 研究在传感器校准或图像方向等应用中常见的旋转偏移问题，利用OT对齐分布以改进模型迁移。

Method: 结合K-means聚类、OT和SVD，估计旋转角度并调整回归模型，特别适用于目标域数据稀疏的情况。

Result: 在$\mathbb{R}^2$中，使用p-norm成本（$p \ge 2$）时，OT映射能恢复底层旋转。

Conclusion: 该方法为几何变换下基于OT的模型适应提供了理论和实践支持。
...
### [172] [Fairness-aware Bayes optimal functional classification](https://arxiv.org/abs/2505.09471)
*Xiaoyu Hu,Gengyu Xue,Zhenhua Lin,Yi Yu*

Main category: stat.ML

TL;DR: 论文提出了一种公平性约束下的功能数据分类框架，设计了Fair-FLDA算法，确保分类器的公平性并控制超额风险。


<details>
  <summary>Details</summary>
Motivation: 研究功能数据分类中的公平性问题，解决现有方法在无限维功能空间中的挑战。

Method: 提出Fair-FLDA算法，通过组间阈值调整实现公平性，并基于同方差高斯过程设计。

Result: 在弱结构假设下，理论证明了算法的公平性和超额风险控制，实验验证了其有效性。

Conclusion: Fair-FLDA在功能数据分类中实现了公平性与性能的平衡，填补了FLDA超额风险控制的理论空白。
...
### [173] [Reinforcement Learning for Individual Optimal Policy from Heterogeneous Data](https://arxiv.org/abs/2505.09496)
*Rui Miao,Babak Shahbaba,Annie Qu*

Main category: stat.ML

TL;DR: 论文提出了一种针对异构数据的个性化离线强化学习框架P4L，通过潜在变量建模个体Q函数，并在弱覆盖假设下实现快速收敛。


<details>
  <summary>Details</summary>
Motivation: 传统离线强化学习方法在处理异构数据时表现不佳，无法为不同个体提供最优策略。

Method: 提出基于个体潜在变量的异构MDP模型，开发P4L算法，通过惩罚悲观策略优化个体Q函数。

Result: P4L在仿真和实际数据中表现优于现有方法，且在弱覆盖假设下具有快速收敛性。

Conclusion: P4L为异构数据下的离线强化学习提供了一种高效且个性化的解决方案。
...
### [174] [Deep-SITAR: A SITAR-Based Deep Learning Framework for Growth Curve Modeling via Autoencoders](https://arxiv.org/abs/2505.09506)
*María Alejandra Hernández,Oscar Rodriguez,Dae-Jin Lee*

Main category: stat.ML

TL;DR: 提出了一种基于自编码器的深度学习框架Deep-SITAR，结合B样条模型估计SITAR模型，用于预测个体生长轨迹。


<details>
  <summary>Details</summary>
Motivation: 传统SITAR模型虽能捕捉生长复杂性，但缺乏预测新个体的灵活性。

Method: 采用自编码器架构，编码器估计随机效应，解码器基于B样条拟合，类似经典SITAR模型。

Result: Deep-SITAR无需重新估计模型即可预测新个体的随机效应。

Conclusion: Deep-SITAR结合了深度学习的灵活性和传统模型的解释性，为生长轨迹预测提供了高效方法。
...
### [175] [Adaptively-weighted Nearest Neighbors for Matrix Completion](https://arxiv.org/abs/2505.09612)
*Tathagata Sadhukhan,Manit Paul,Raaz Dwivedi*

Main category: stat.ML

TL;DR: AWNN是一种自适应加权最近邻方法，用于矩阵补全，解决了传统最近邻方法中权重和半径选择的问题。


<details>
  <summary>Details</summary>
Motivation: 传统最近邻方法在权重和半径选择上缺乏系统性，AWNN旨在通过自适应加权解决这一问题。

Method: AWNN通过平衡加权最近邻回归中的偏差-方差权衡，自适应选择权重和半径。

Result: 理论分析和合成实验验证了AWNN的有效性。

Conclusion: AWNN为矩阵补全提供了一种无需交叉验证的系统性解决方案。
...
### [176] [A Method for Assisting Novices Creating Class Diagrams Based on the Instructor's Class Layout](https://arxiv.org/abs/2505.09116)
*Yuta Saito,Takehiro Kokubu,Takafumi Tanaka,Atsuo Hazeyama,Hiroaki Hashiura*

Main category: cs.SE

TL;DR: 论文提出了一种方法，通过自动将学习者的类图布局转换为教师的标准布局，并提供正确性反馈，以支持建模练习。


<details>
  <summary>Details</summary>
Motivation: 学习者在建模练习中常出现元素缺失和布局差异问题，需要有效支持。

Method: 自动转换学习者类图布局至教师标准布局，并提供正确性反馈。

Result: 工具实现与评估表明，自动布局转换对学习者反馈有效。

Conclusion: 该方法能有效支持学习者在建模练习中的改进。

Abstract: Nowadays, modeling exercises on software development objects are conducted in
...
### [177] [AI-Mediated Code Comment Improvement](https://arxiv.org/abs/2505.09021)
*Maria Dhakal,Chia-Yi Su,Robert Wallace,Chris Fakhimi,Aakash Bansal,Toby Li,Yu Huang,Collin McMillan*

Main category: cs.SE

TL;DR: 本文提出了一种基于AI的工具改进代码注释质量的方法，通过LLM重写注释，并蒸馏模型以支持本地运行。


<details>
  <summary>Details</summary>
Motivation: 改进代码注释质量，通过AI工具提升可读性和实用性。

Method: 结合实证研究和定性分析确定质量维度，使用LLM（如GPT-4o）重写注释，并蒸馏为小型本地模型。

Result: 方法显著提升了代码注释质量，且蒸馏模型在本地运行效果良好。

Conclusion: AI工具能有效改进代码注释，支持数据主权，开源数据与代码以促进可复现性。
...
### [178] [Tests as Prompt: A Test-Driven-Development Benchmark for LLM Code Generation](https://arxiv.org/abs/2505.09027)
*Yi Cui*

Main category: cs.SE

TL;DR: WebApp1K是一个新基准，用于评估大语言模型在测试驱动开发任务中的表现，强调从测试用例直接生成代码的能力。


<details>
  <summary>Details</summary>
Motivation: 传统方法依赖自然语言提示，而WebApp1K更贴近实际软件开发实践，测试用例既是提示也是验证。

Method: 包含1000个跨20个应用领域的挑战，评估模型在上下文长度和多功能复杂性约束下生成功能代码的能力。

Result: 研究发现指令遵循和上下文学习对测试驱动开发成功至关重要，超越了通用编码能力或预训练知识。

Conclusion: 该工作强调了测试驱动开发专用基准的实际价值，为提升大语言模型在严格应用驱动编码场景中的能力奠定了基础。
...
### [179] [Variational Prefix Tuning for Diverse and Accurate Code Summarization Using Pre-trained Language Models](https://arxiv.org/abs/2505.09062)
*Junda Zhao,Yuliang Song,Eldan Cohen*

Main category: cs.SE

TL;DR: 论文提出了一种名为Variational Prefix Tuning（VPT）的新方法，用于生成多样且准确的代码摘要，允许用户选择最合适的摘要。该方法结合了条件变分自编码器（CVAE）框架，并以参数高效的方式实现，无需昂贵的模型重训练。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常只为给定源代码生成单一高质量摘要，忽略了生成摘要可能不充分且需要替代选项的场景。

Method: VPT方法将CVAE框架作为模块化组件集成到预训练模型中，通过采样连续嵌入作为前缀来引导生成多样化输出，并使用双标准重排方法优化摘要的多样性和准确性。

Result: 实验评估表明，VPT方法在广泛使用的数据集和最先进的预训练代码摘要模型上表现出色，具有跨模型的适应性。

Conclusion: VPT方法能够有效生成多样且准确的代码摘要，同时避免了昂贵的模型重训练，具有实际应用价值。
...
### [180] [Customizing a Large Language Model for VHDL Design of High-Performance Microprocessors](https://arxiv.org/abs/2505.09610)
...
TL;DR: 论文探讨了在硬件设计中使用LLM，特别是针对VHDL代码解释的开发过程，通过扩展预训练和专家评估提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于填补VHDL代码解释的LLM应用空白，并满足高性能处理器设计组织的需求。

Method: 通过扩展预训练（EPT）和专家评估开发专用LLM，并引入LLM-as-a-judge方法评估模型。

Result: EPT模型的专家评估评分从43%提升至69%，指令调优版本预期可达71%，未来可能提升至85%。
...
### [181] [Probabilistic Wind Power Forecasting via Non-Stationary Gaussian Processes](https://arxiv.org/abs/2505.09026)
*Domniki Ladopoulou,Dat Minh Hong,Petros Dellaportas*

Main category: stat.AP

TL;DR: 提出了一种基于广义谱混合（GSM）核的非平稳高斯过程（GP）模型，用于风功率的概率预测，优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 风功率的准确概率预测对电网稳定和可再生能源高效整合至关重要，但传统方法依赖平稳核，无法捕捉风功率的非平稳特性。

Method: 采用非平稳GP框架，结合GSM核，以捕捉风功率数据中的时变模式和异方差行为。

Result: 在真实SCADA数据上验证，GSM核模型在短期预测中表现优于传统径向基函数和谱混合核。

Conclusion: 非平稳GP模型在风功率预测中具有实际应用价值，强调了建模非平稳性的必要性。
...
### [182] [Bounding Neyman-Pearson Region with $f$-Divergences](https://arxiv.org/abs/2505.08899)
...
### [183] [Statistical Decision Theory with Counterfactual Loss](https://arxiv.org/abs/2505.08908)
*Benedikt Koch,Kosuke Imai*

Main category: math.ST

TL;DR: 论文扩展了经典统计决策理论，引入反事实损失以评估决策质量，解决了仅依赖观察结果的局限性。


<details>
  <summary>Details</summary>
Motivation: 经典决策理论仅基于观察结果评估治疗选择，无法衡量决策相对于可行替代方案的质量。

Method: 通过假设强可忽略性，证明反事实风险可识别当且仅当反事实损失函数在潜在结果中具有可加性。

Result: 在强可忽略性假设下，可加性反事实损失函数能识别风险，且在多治疗选项下可能产生不同于标准损失函数的推荐。

Conclusion: 反事实损失扩展了决策理论，为多治疗选项问题提供了更全面的评估框架。
...
### [184] [A Comparative Review of RNA Language Models](https://arxiv.org/abs/2505.09087)
*He Wang,Yikun Zhang,Jie Chen,Jian Zhan,Yaoqi Zhou*

Main category: q-bio.BM

TL;DR: 论文分析了13种RNA语言模型（LMs）在RNA二级结构和功能分类的零样本预测中的表现，发现性能不平衡，需更均衡的无监督训练。


<details>
  <summary>Details</summary>
Motivation: RNA语言模型在结构和功能推断中的潜力受到关注，但缺乏统一标准比较。

Method: 将RNA LMs分为三类（多RNA类型预训练、特定目的RNA、RNA与DNA/蛋白质统一模型），并与DNA和蛋白质LMs对比。

Result: 二级结构预测表现好的模型在功能分类中表现较差，反之亦然。

Conclusion: 需更均衡的无监督训练以提升模型性能。
...
### [185] [A Retrieval-Augmented Generation Framework for Academic Literature Navigation in Data Science](https://arxiv.org/abs/2412.15404)
...
<details>
  <summary>Details</summary>
Motivation: 在数据科学领域快速发展的背景下，高效导航海量学术文献对决策和创新至关重要。

Method: 结合GROBID技术、微调嵌入模型、语义分块和摘要优先检索方法，提升信息检索的准确性和相关性。

Result: 使用RAGAS框架评估显示，系统在上下文相关性等关键指标上有显著提升，有效减少信息过载。

Conclusion: 该增强型RAG系统有望改变数据科学领域的学术探索方式，推动研究和创新流程的进步。
...
### [186] [Focus, Merge, Rank: Improved Question Answering Based on Semi-structured Knowledge Bases](https://arxiv.org/abs/2505.09246)
*Derian Boer,Stephen Roth,Stefan Kramer*

Main category: cs.IR

TL;DR: FocusedRetriever是一个基于半结构化知识库（SKB）的多跳问答框架，通过结合结构化与非结构化数据，利用LLM和向量相似性搜索等技术，在STaRK基准测试中表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决现有机器学习模型和交互系统通常仅依赖结构化或非结构化数据的局限性，通过SKB结合两者，提升知识访问和利用效率。

Method: 框架包括VSS实体搜索、LLM生成Cypher查询、成对重排序等模块，利用LLM提取关系事实和实体属性，并通过节点集连接和向量相似性搜索优化答案候选。

Result: 在STaRK基准测试的三个数据集上均优于现有方法，平均首次命中率比次优方法高25.7%。

Conclusion: FocusedRetriever展示了SKB在多跳问答中的潜力，未来可通过微调LLM进一步优化性能。代码已开源。
...
### [187] [Diffusion Recommender Models and the Illusion of Progress: A Concerning Study of Reproducibility and a Conceptual Mismatch](https://arxiv.org/abs/2505.09364)
*Michael Benigni,Maurizio Ferrari Dacrema,Dietmar Jannach*

Main category: cs.IR

TL;DR: 研究发现，2023-2024年SIGIR会议上发表的基于扩散模型的推荐系统方法存在方法论问题，实际性能不如简单模型，且与推荐任务不匹配。


<details>
  <summary>Details</summary>
Motivation: 探讨当前推荐系统研究中是否存在方法论问题，特别是扩散模型是否真的推动了技术进步。

Method: 通过复现2023-2024年SIGIR会议上发表的四种扩散模型推荐方法，并与简单基线模型对比。

Result: 扩散模型在推荐任务中表现不佳，性能不如简单模型，且与任务特性不匹配。

Conclusion: 呼吁提高研究严谨性，改变当前推荐系统研究的发表文化。
...
### [188] [SALM: A Multi-Agent Framework for Language Model-Driven Social Network Simulation](https://arxiv.org/abs/2505.09081)
*Gaurav Koley*

Main category: cs.SI

TL;DR: 本文提出了一种名为SALM的新型框架，通过将语言模型（LMs）整合到社交网络模拟中，实现了多智能体场景下的时间稳定性。主要贡献包括分层提示架构、基于注意力的记忆系统以及对个性稳定性的形式化界定。


<details>
  <summary>Details</summary>
Motivation: 传统的基于规则的社交系统建模方法难以捕捉复杂动态，本文旨在通过语言模型提升模拟的上下文理解能力。

Method: SALM框架采用分层提示架构和基于注意力的记忆系统，显著减少了令牌使用并优化了内存增长。

Result: 实验验证表明，SALM在SNAP ego网络上实现了长期稳定的社交现象模拟，行为保真度得到经验验证。

Conclusion: SALM是首个基于LLM的框架，能够有效模拟长期社交现象，同时保持行为保真度。
...
### [189] [Visibility and Influence in Digital Social Relations: Towards a New Symbolic Capital?](https://arxiv.org/abs/2505.08797)
*F. Annaki,S. Ouassou,S. Igamane*

Main category: cs.SI

TL;DR: 研究探讨数字社交关系中可见性与影响力的动态，揭示新型象征资本的形成。


<details>
  <summary>Details</summary>
Motivation: 探索数字社交中可见性与影响力的关系及其对新型象征资本的影响。

Method: 采用混合方法设计，结合20位数字活跃个体的半结构化访谈和社交媒体数据分析。

Result: 发现可见性受内容质量、网络规模和互动策略影响，影响力则依赖可信度、权威和信任。

Conclusion: 提出基于在线可见性、影响力和声誉的新型象征资本，强调需更新社会理论以适应数字变革。

Abstract: This study explores the dynamics of visibility and influence in digital
...
### [190] [The Niche Connectivity Paradox: Multichrome Contagions Overcome Vaccine Hesitancy more effectively than Monochromacy](https://arxiv.org/abs/2505.09605)
*Ho-Chun Herbert Chang,Feng Fu*

Main category: cs.SI

TL;DR: 论文分析了疫苗犹豫的动态复杂性，提出多色传染作为干预目标，通过Twitter数据识别摇摆人群，并探讨了利用多色传染促进疫苗态度的策略。


<details>
  <summary>Details</summary>
Motivation: 疫苗犹豫导致疫苗可预防疾病的复燃，现有研究常将人群简单分为支持或反对疫苗，忽略了态度的动态变化。

Method: 利用Twitter数据（1.35亿用户和35亿条推文）构建多路情感网络，识别摇摆人群及其信息消费模式。

Result: 摇摆人群更中立、话题广泛且网络位置中心化，多色传染干预可有效促进疫苗态度。

Conclusion: 多色传染的协同效应可用于网络干预，克服疫苗犹豫。

Abstract: The rise of vaccine hesitancy has caused a resurgence of vaccine-preventable
...
### [191] [Statistical Mean Estimation with Coded Relayed Observations](https://arxiv.org/abs/2505.09098)
*Yan Hao Ling,Zhouhao Yang,Jonathan Scarlett*

Main category: cs.IT

TL;DR: 论文研究了通过中继（“教师”）和信道传输的统计均值估计问题，在大偏差机制下建立了紧致的误差指数，优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 探索在样本通过信道间接传输的情况下，如何优化统计均值估计的误差指数。

Method: 研究伯努利源和二进制对称信道，随后推广到次高斯、重尾分布及任意离散无记忆信道。

Result: 在大偏差机制下，建立了紧致的误差指数，优于两种基线方法。

Conclusion: 通过信道传输的统计均值估计在大偏差机制下可以实现最优误差指数，基线方法表现次优。
...
### [192] [Enhanced Photonic Chip Design via Interpretable Machine Learning Techniques](https://arxiv.org/abs/2505.09266)
...
<details>
  <summary>Details</summary>
Motivation: 解决逆向设计优化方法（如机器学习和损失函数优化）的黑箱特性，以提升光子组件设计的透明度和性能。

Method: 采用局部可解释模型无关解释（LIME）技术，分析逆向设计优化过程，特别针对双模复用器。

Result: LIME提供的洞察帮助优化初始条件，直接提升了器件性能，表明可解释性技术不仅能解释模型，还能指导设计。

Conclusion: 可解释性技术能揭示逆向设计中的潜在模式，从而开发出性能更优的光子组件。
...
### [193] [A New Tractable Description Logic under Categorical Semantics](https://arxiv.org/abs/2505.08916)
*Chan Le Duc,Ludovic Brieulle*

Main category: cs.LO

TL;DR: 论文提出了一种新的EL逻辑扩展，通过弱化否定语义来保留可处理性，同时增强表达能力。


<details>
  <summary>Details</summary>
Motivation: 生物医学本体中常使用否定知识（如“lacks_part”），但现有逻辑无法直接表达此类语义，且添加否定会导致逻辑不可处理。

Method: 引入SH描述逻辑的分类语义，通过弱化析取和全称限制的语义来恢复可处理性。

Result: 提出的弱化语义逻辑比带有底部概念、传递角色和角色包含的EL更具表达力。

Conclusion: 该扩展能够在保留可处理性的同时有效表达否定知识。
...
### [194] [Equilibrium Propagation for Learning in Lagrangian Dynamical Systems](https://arxiv.org/abs/2505.07363)
*Serge Massar*

Main category: nlin.CD

TL;DR: 提出了一种基于拉格朗日力学的动态系统训练方法，利用平衡传播原理，通过动作极值化实现高效参数更新。


<details>
  <summary>Details</summary>
Motivation: 扩展平衡传播方法至动态轨迹训练，解决传统时间反向传播的复杂性，适用于周期性边界条件或固定初始/终止状态的系统。

Method: 通过轻微扰动轨迹至目标状态，测量参数共轭变量的响应，实现参数更新，无需显式时间反向传播。

Result: 适用于周期性边界条件的系统，并可推广至耗散系统，展示了半经典量子平衡传播的极限情况。

Conclusion: 该方法为动态系统训练提供了一种高效且理论严谨的替代方案，尤其适用于特定边界条件的场景。
...
### [195] [Depth-Based Local Center Clustering: A Framework for Handling Different Clustering Scenarios](https://arxiv.org/abs/2505.09516)
*Siyi Wang,Alexandre Leblanc,Paul D. McNicholas*

Main category: stat.ME

TL;DR: 论文提出了一种基于数据深度的局部中心聚类方法（DLCC），通过局部数据深度识别多模态数据中的局部中心和不同形状的簇，并引入新的内部度量标准评估聚类性能。


<details>
  <summary>Details</summary>
Motivation: 现有聚类方法通常针对特定场景设计，存在局限性，尤其是在处理多模态和非凸簇时效果不佳。

Method: DLCC利用局部数据深度，基于数据子集识别局部中心和不同形状的簇，并提出了基于密度的内部度量标准。

Result: DLCC能够有效克服传统聚类方法的局限性，适用于多种应用场景。

Conclusion: DLCC是一种灵活的聚类方法，提升了多模态和非凸簇数据的分析能力。
...
### [196] [Scalable Computations for Generalized Mixed Effects Models with Crossed Random Effects Using Krylov Subspace Methods](https://arxiv.org/abs/2505.09552)
*Pascal Kündig,Fabio Sigrist*

Main category: stat.ME

TL;DR: 提出基于Krylov子空间的新方法，解决高维交叉随机效应模型的计算瓶颈，显著提升速度和稳定性。


<details>
  <summary>Details</summary>
Motivation: 高维交叉随机效应模型中，传统基于Cholesky分解的计算方法速度过慢，亟需高效替代方案。

Method: 开发Krylov子空间方法，分析预条件子，推导收敛性结果，优化预测方差计算。

Result: 实验显示方法比Cholesky快约两个数量级，软件实现比lme4和glmmTMB快至万倍且更稳定。

Conclusion: 新方法显著提升计算效率，已集成于GPBoost库，支持Python和R。
...
TL;DR: DPN-GAN是一种新型GAN架构，通过引入周期性激活函数和可变形卷积，解决了传统GAN在音频生成中的分辨率限制和模式崩溃问题，显著提升了生成音频的质量和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统GAN在音频生成中依赖带宽受限的梅尔频谱图，导致生成分辨率低且易出现模式崩溃，限制了音频生成的质量。

Method: 提出DPN-GAN，结合基于核的周期性ReLU激活函数和可变形卷积操作，支持多分辨率生成，并优化判别器网络以提升音频质量。

Result: 实验表明，DPN-GAN在多种数据集上表现优异，生成音频质量高，且对噪声和分布外数据具有鲁棒性。

Conclusion: DPN-GAN在音频生成任务中优于现有GAN架构，展示了其高效性和适应性。
...
TL;DR: VtaD 2025挑战赛旨在通过比较方式解释人声音色属性，使用感官描述符（如明亮、粗糙、柔和等）量化人声印象，并计划在2025年10月的NCMMSC2025会议上展示成果。


<details>
  <summary>Details</summary>
Motivation: 研究人声音色的独特属性，并通过感官描述符将其量化，以更好地理解和比较不同声音的特征。

Method: 通过比较两段声音在特定描述符维度上的强度，解释音色属性。

Result: 挑战赛将于2025年5月启动，并在10月的NCMMSC2025会议上展示特别提案。

Conclusion: VtaD 2025挑战赛为声音音色属性的研究提供了一个量化框架，并计划通过会议展示进一步推动该领域的发展。
...
TL;DR: 提出了一种低计算量的方法，用于关键词检测（KWS）中预训练神经网络的连续噪声适应，仅需单次学习和一个训练周期，显著提升了在低信噪比下的性能。


<details>
  <summary>Details</summary>
Motivation: 标准KWS系统在嵌入式设备上运行时性能下降，需要动态适应能力以应对噪声、用户差异等挑战。

Method: 采用低计算量的连续噪声适应方法，仅需1-shot学习和一个训练周期，适用于资源受限设备。

Result: 在多种噪声环境下，适应后的模型性能优于预训练模型，尤其在低信噪比（≤18 dB）下，准确率提升4.9%至46.0%。

Conclusion: 该方法轻量高效，适合在资源受限设备上部署，显著提升了KWS系统的噪声鲁棒性。
...
### [200] [Accelerating Machine Learning Systems via Category Theory: Applications to Spherical Attention for Gene Regulatory Networks](https://arxiv.org/abs/2505.09326)
*Vincent Abbott,Kotaro Kamiya,Gerard Glowacki,Yu Atsumi,Gioele Zardini,Yoshihiro Maruyama*

Main category: math.CT

TL;DR: 论文提出了一种基于神经电路图的方法，用于改进深度学习架构，并开发了一种新型的球形注意力算法，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前自动编译方法效率低下，而高效算法需要多年人工开发。论文旨在通过神经电路图提供一种系统化方法，改进深度学习架构。

Method: 使用基于范畴论的神经电路图，提出球形注意力算法，用L2范数替代SoftMax，并开发了高效的FlashSign内核。

Result: 球形注意力算法克服了标准注意力的瓶颈，FlashSign内核性能与FlashAttention相当，且比PyTorch快3.6倍。
...
### [201] [InvDesFlow-AL: Active Learning-based Workflow for Inverse Design of Functional Materials](https://arxiv.org/abs/2505.09203)
...
<details>
  <summary>Details</summary>
Motivation: 开发具有特定功能材料的逆向设计方法对可再生能源、催化、能源存储和碳捕获等领域至关重要，但现有方法成功率较低。

Method: 提出InvDesFlow-AL框架，结合主动学习策略迭代优化材料生成过程，逐步引导至目标性能。

Result: 在晶体结构预测中，InvDesFlow-AL的RMSE为0.0423 Å，性能提升32.96%；成功设计出低形成能和低Ehull材料，并发现Li₂AuH₆为140 K超导体。

Conclusion: InvDesFlow-AL通过主动学习显著加速了材料发现和逆向设计，为材料科学提供了有效工具。
...
### [202] [Bridging Theory and Experiment in Materials Discovery: Machine-Learning-Assisted Prediction of Synthesizable Structures](https://arxiv.org/abs/2505.09161)
...
<details>
  <summary>Details</summary>
Motivation: 解决传统能量驱动的晶体结构预测方法难以识别实验可合成的亚稳态材料的问题，缩小理论与实验之间的差距。

Method: 整合对称性引导的结构推导与基于Wyckoff编码的机器学习模型，结合从头计算和可合成性评估模型。

Result: 成功重现13种已知XSe结构，筛选出92,310种潜在可合成结构，并识别出8种热力学有利的Hf-X-O结构。

Conclusion: 该框架为机器学习辅助的无机材料合成提供了新范式，有望推动功能材料的定向发现。
...
### [203] [WaLLM -- Insights from an LLM-Powered Chatbot deployment via WhatsApp](https://arxiv.org/abs/2505.08894)
*Hiba Eltigani,Rukhshan Haroon,Asli Kocak,Abdullah Bin Faisal,Noah Martin,Fahad Dogar*

Main category: cs.HC

TL;DR: WaLLM是一个基于WhatsApp的AI聊天机器人，旨在通过提供信息查询和互动功能（如每日热门问题、奖励系统等）弥合发展中国家的数字鸿沟。


<details>
  <summary>Details</summary>
Motivation: 生成式AI（如ChatGPT）在教育等领域有广泛应用，但发展中国家因数字鸿沟难以普及。WaLLM通过WhatsApp这一普及平台提供AI服务。

Method: 开发WaLLM并分析6个月内的用户日志（14.7K查询，100用户），研究用户行为和互动模式。

Result: 55%查询为事实信息，健康和福祉最热门（28%）。用户互动集中在每日热门问题后24小时内，使用排行榜的用户互动频率高3倍。

Conclusion: 需针对文化定制、优化界面设计，并校准用户对AI的信任，以提升发展中国家AI服务的有效性。
...
### [204] [EcoSphere: A Decision-Support Tool for Automated Carbon Emission and Cost Optimization in Sustainable Urban Development](https://arxiv.org/abs/2505.09054)
*Siavash Ghorbany,Ming Hu,Siyuan Yao,Matthew Sisk,Chaoli Wang*

Main category: cs.HC

TL;DR: EcoSphere是一款创新软件，用于评估和平衡城市规划中的隐含碳和运营碳排放与成本，通过高分辨率数据和AI技术提供政策模拟和可持续决策支持。


<details>
  <summary>Details</summary>
Motivation: 建筑业是全球温室气体排放的主要来源之一，隐含碳是关键组成部分。研究旨在通过技术手段帮助城市实现碳中和目标。

Method: 结合国家结构清单的高分辨率数据、计算机视觉和自然语言处理技术，对建筑进行分类，并通过政策情景模拟提供决策支持。

Result: 在芝加哥和印第安纳波利斯的案例研究中，EcoSphere成功评估了政策对碳排放和成本的影响，支持数据驱动的碳中和进程。

Conclusion: EcoSphere为政策制定者和非专业人士提供了可持续城市发展的可行见解，展示了决策的环境和财务结果。

Abstract: The construction industry is a major contributor to global greenhouse gas
...
### [205] [Performance Gains of LLMs With Humans in a World of LLMs Versus Humans](https://arxiv.org/abs/2505.08902)
...
TL;DR: 论文呼吁停止将LLMs与人类专家直接比较，转而研究如何安全地在临床环境中协同使用LLMs与人类。


<details>
  <summary>Details</summary>
Motivation: 当前研究常将LLMs与人类专家比较，但“专家”定义模糊且LLMs更新迅速，可能威胁患者安全。

Method: 提出需开发策略，使人类与LLMs高效协作，而非对立比较。

Result: 强调LLMs与人类协同工作的必要性，而非竞争关系。

Conclusion: 未来研究应聚焦于LLMs在临床环境中的安全使用策略，促进人机共生。
...
### [206] [Tracing the Invisible: Understanding Students' Judgment in AI-Supported Design Work](https://arxiv.org/abs/2505.08939)
*Suchismita Naik,Prakash Shukla,Ike Obi,Jessica Backus,Nancy Rasche,Paul Parsons*

Main category: cs.HC

TL;DR: 研究分析了33个学生团队在HCI设计课程中使用AI工具时的判断类型，发现了传统设计判断和新出现的判断（如责任分配和可靠性判断），揭示了AI如何增加设计推理的复杂性。


<details>
  <summary>Details</summary>
Motivation: 探讨学生在设计工作流中如何将AI工具视为协作伙伴，并分析其判断类型。

Method: 通过分析33个学生团队的反思内容，识别设计判断的类型。

Result: 发现了传统设计判断（如工具性、欣赏性、质量判断）和新类型（责任分配和可靠性判断）。

Conclusion: 生成式AI为设计推理增加了新复杂性，学生需思考如何依赖AI，研究为理解学生与AI的共创意义提供了概念框架。
...
### [207] [PreCare: Designing AI Assistants for Advance Care Planning (ACP) to Enhance Personal Value Exploration, Patient Knowledge, and Decisional Confidence](https://arxiv.org/abs/2505.09115)
...
TL;DR: PreCare是一个结合AI助手的在线ACP平台，旨在弥补传统在线ACP缺乏个性化咨询的不足，显著提升用户对个人价值观的探索、知识获取和决策信心。


<details>
  <summary>Details</summary>
Motivation: 在线ACP虽然便捷，但缺乏临床咨询的个性化价值探索和即时澄清决策后果的优势。

Method: 通过两项形成性研究（观察ACP团队和访谈ACP网站用户）设计PreCare，并进行了可用性研究和比较评估。

Result: PreCare的SUS评分为优秀，AI助手显著提升用户对价值观的探索、知识获取和决策信心，92%的参与者更偏好PreCare。

Conclusion: PreCare成功结合AI技术弥补了在线ACP的不足，提供了更有效的ACP支持。
...
### [208] [An Initial Exploration of Default Images in Text-to-Image Generation](https://arxiv.org/abs/2505.09166)
*Hannu Simonen,Atte Kiviniemi,Jonas Oppenlaender*

Main category: cs.HC

TL;DR: 本文研究了文本到图像生成（TTI）中的“默认图像”现象，即在输入提示包含未知术语时模型生成的相似图像。通过系统实验和用户调查，探讨了默认图像对用户满意度的影响，并提出了未来研究方向。


<details>
  <summary>Details</summary>
Motivation: TTI模型在输入提示包含未知术语时会生成相似的“默认图像”，研究这一现象有助于改进TTI技术和提示工程。

Method: 通过系统方法设计输入提示以触发默认图像，进行实验和小规模消融研究，并开展用户满意度调查。

Result: 揭示了默认图像的存在及其对用户满意度的影响，为理解TTI中的默认图像奠定了基础。

Conclusion: 默认图像的研究为改进TTI技术提供了新视角，未来需进一步探索其机制和解决方案。
...
### [209] [Educational impacts of generative artificial intelligence on learning and performance of engineering students in China](https://arxiv.org/abs/2505.09208)
*Lei Fan,Kunyang Deng,Fangxue Liu*

Main category: cs.HC

TL;DR: 研究探讨了生成式AI对中国工程学生的学习和教育的影响，发现其对学习效率和创造力有积极影响，但也存在准确性和领域可靠性的挑战。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI的快速发展，其在高等教育中的应用潜力备受关注，研究旨在了解其对工程学生学习和教育的影响。

Method: 通过调查148名来自中国不同地区和工程学科的学生，分析生成式AI的使用频率、场景、影响、挑战及未来前景。

Result: 超过半数学生认为生成式AI提升了学习效率和创造力，但对其准确性和学术表现的实际影响表示担忧。

Conclusion: 研究为生成式AI在工程教育中的整合提供了学生视角的建议，强调了其潜力与挑战。
...
### [210] [Security of Internet of Agents: Attacks and Countermeasures](https://arxiv.org/abs/2505.08807)
...
Method: 通过分析IoA架构及其与传统网络的差异，聚焦身份认证威胁、跨代理信任、实体安全和隐私风险四大方面。

Result: 综述了现有和新兴的防御机制，并指出持续存在的挑战。

Conclusion: 提出了推动弹性且隐私保护的IoA生态系统发展的开放研究方向。
...
### [211] [MixBridge: Heterogeneous Image-to-Image Backdoor Attack through Mixture of Schrödinger Bridges](https://arxiv.org/abs/2505.08809)
*Shixi Qin,Zhiyong Yang,Shilong Bao,Shi Wang,Qianqian Xu,Qingming Huang*

Main category: cs.CR

TL;DR: 本文提出MixBridge框架，用于在扩散Schrödinger桥模型中植入多异质后门触发器，解决现有方法局限于单攻击场景和高斯噪声输入的问题。


<details>
  <summary>Details</summary>
Motivation: 现有后门方法仅适用于单攻击场景和高斯噪声输入模型，无法满足复杂输入分布的需求。

Method: 提出MixBridge框架，通过直接训练带毒图像对植入后门触发器，无需修改随机微分方程；采用Divide-and-Merge策略和Weight Reallocation Scheme（WRS）解决多触发器冲突问题。

Result: 实验证明MixBridge在多种生成任务中有效，且WRS提升了后门隐蔽性。

Conclusion: MixBridge为桥模型后门行为研究提供了灵活工具，Divide-and-Merge策略成功解决了多触发器冲突问题。
...
### [212] [Machine Learning-Based Detection of DDoS Attacks in VANETs for Emergency Vehicle Communication](https://arxiv.org/abs/2505.08810)
*Bappa Muktar,Vincent Fono,Adama Nouboukpo*

Main category: cs.CR

TL;DR: 该论文提出了一种用于高速公路VANET环境中检测DDoS攻击的框架，通过合成数据集和多种分类器验证，XGBoost和CatBoost表现最佳（F1-score 96%）。
...
Method: 使用NS-3和SUMO构建合成数据集，结合真实交通数据，通过预处理和特征工程，评估11种分类器。

Result: XGBoost和CatBoost表现最佳，F1-score达96%。
...
### [213] [Federated Large Language Models: Feasibility, Robustness, Security and Future Directions](https://arxiv.org/abs/2505.08830)
*Wenhao Jiang,Yuchuan Luo,Guilin Deng,Silong Chen,Xu Yang,Shihong Wu,Xinwen Gao,Lin Liu,Shaojing Fu*

Main category: cs.CR

TL;DR: 本文综述了联邦大语言模型（FLLM）的最新进展，重点探讨了可行性、鲁棒性、安全性和未来方向四大挑战，并提出了增强系统鲁棒性和安全性的研究方向。


<details>
  <summary>Details</summary>
Motivation: 结合大语言模型（LLM）和联邦学习（FL）可以解决分布式数据隐私和数据孤岛问题，但面临通信计算开销、异构性及隐私安全等挑战。

Method: 通过全面综述现有研究，分析FLLM的可行性、鲁棒性增强方法、隐私安全风险及防御机制，并探讨未来方向如少样本学习、机器遗忘和IP保护。

Result: 当前研究主要集中在FLLM的可行性，未来趋势将更关注系统鲁棒性和安全性。

Conclusion: FLLM领域亟需进一步研究以应对FL与LLM结合带来的独特挑战，特别是在鲁棒性和安全性方面。
...
### [214] [Robustness Analysis against Adversarial Patch Attacks in Fully Unmanned Stores](https://arxiv.org/abs/2505.08835)
...
<details>
  <summary>Details</summary>
Motivation: 随着无人商店的普及，其AI系统的安全漏洞（如对抗性补丁攻击）可能导致盗窃和库存问题，亟需研究其影响和防御方法。

Method: 研究了三种对抗性补丁攻击（隐藏、创建和修改攻击），提出新的颜色直方图相似性损失函数和边界框评估指标，并在数字和物理环境中测试攻击效果。

Result: 对抗性补丁攻击在物理环境中显著破坏物体检测模型，阴影攻击在无模型参数访问时也能提高成功率，现有防御机制在实时检测中存在不足。

Conclusion: 研究强调了开发更鲁棒的防御策略的必要性，以提升无人商店中物体检测模型的抗攻击能力。
...
### [215] [On the interplay of Explainability, Privacy and Predictive Performance with Explanation-assisted Model Extraction](https://arxiv.org/abs/2505.08847)
*Fatima Ezzeddine,Rinad Akel,Ihab Sbeity,Silvia Giordano,Marc Langheinrich,Omran Ayoub*

Main category: cs.CR

TL;DR: 本文研究了在MLaaS平台中，如何通过差分隐私（DP）技术平衡模型性能、隐私和可解释性，以应对模型提取攻击（MEA）。


<details>
  <summary>Details</summary>
Motivation: 随着MLaaS的普及和XAI的整合，攻击者可能利用反事实解释（CFs）进行MEA，因此需要探索隐私保护方法。

Method: 评估了两种DP策略：一种在分类模型训练时实施，另一种在CF生成时实施。

Result: 研究发现DP技术可以有效缓解CFs导致的MEA，但需要在性能、隐私和可解释性之间权衡。

Conclusion: DP是应对CFs相关MEA的有效手段，但需根据具体场景选择合适的实施策略。
...
### [216] [Improved Algorithms for Differentially Private Language Model Alignment](https://arxiv.org/abs/2505.08849)
*Keyu Chen,Hao Tang,Qinglin Liu,Yizhao Xu*

Main category: cs.CR

TL;DR: 提出了一种隐私保护的语言模型对齐算法，结合DPO和RLHF，性能优于现有方法，提升对齐质量15%。


<details>
  <summary>Details</summary>
Motivation: 语言模型对齐涉及敏感数据，现有结合差分隐私的方法性能有限，需改进。

Method: 提出新算法（如DP-AdamW），结合DPO和RLHF，分析隐私预算与模型效果。

Result: 在中等隐私预算下，DP-AdamW结合DPO提升对齐质量15%，性能最优。

Conclusion: 新算法在隐私保护与对齐效果间取得平衡，提供优化指南。
...
### [217] [Optimized Couplings for Watermarking Large Language Models](https://arxiv.org/abs/2505.08878)
*Dor Tsur,Carol Xuan Long,Claudio Mayrink Verdun,Hsiang Hsu,Haim Permuter,Flavio P. Calmon*

Main category: cs.CR

TL;DR: 本文分析了大型语言模型（LLM）生成文本的水印技术，探讨了水印检测能力与文本质量失真之间的权衡，并提出了一种最优耦合和随机化策略。


<details>
  <summary>Details</summary>
Motivation: 随着LLM生成文本的能力接近人类水平，开发不显著干扰文本质量的水印技术变得重要。本文旨在分析水印设计中的关键问题。

Method: 通过假设检验和侧信息视角，提出了一种耦合侧信息与LLM词汇随机分区的策略，并在最小熵约束下分析了最优方案。

Result: 给出了检测率的闭式表达式，并通过数值实验比较了所提方案与现有方法在合成数据和实际LLM水印中的表现。

Conclusion: 本文提出的水印方案在检测能力和文本质量之间实现了最优权衡，为LLM水印设计提供了理论支持。
...
### [218] [TokenProber: Jailbreaking Text-to-image Models via Fine-grained Word Impact Analysis](https://arxiv.org/abs/2505.08804)
*Longtian Wang,Xiaofei Xie,Tianlin Li,Yuhan Zhi,Chao Shen*

Main category: cs.CR

TL;DR: TokenProber是一种用于评估T2I模型拒绝机制鲁棒性的方法，通过生成对抗性提示，揭示安全检查器的漏洞。
...
Method: 通过敏感性感知的差异测试，分析提示中的关键词，生成对抗性提示以绕过安全检查。

Result: TokenProber在5个安全检查器和3个T2I模型上表现优异，绕过率平均提高54%。

Conclusion: TokenProber能有效发现拒绝机制的鲁棒性问题，为改进安全措施提供依据。
...
### [219] [Self-Supervised Transformer-based Contrastive Learning for Intrusion Detection Systems](https://arxiv.org/abs/2505.08816)
*Ippokratis Koukoulis,Ilias Syrigos,Thanasis Korakis*

Main category: cs.CR

TL;DR: 论文提出了一种基于Transformer编码器的自监督对比学习方法，用于提高入侵检测系统的泛化能力，显著优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 随着零日攻击的频率和严重性增加，传统签名式入侵检测系统（IDS）的局限性凸显，需要更先进的机器学习方法。然而，现有方法依赖标记数据且泛化能力不足。

Method: 采用自监督对比学习，结合数据增强和Transformer架构，直接从原始数据包序列中学习流量表示，避免手工特征提取。

Result: 在异常检测任务中，AUC评分比现有方法高3%（数据集内）和20%（数据集间）。在有限标记数据下，监督学习性能提升1.5% AUC。

Conclusion: 该方法显著提升了入侵检测的泛化能力和性能，尤其在数据稀缺或跨域场景下表现优异。
...
### [220] [Adaptive Security Policy Management in Cloud Environments Using Reinforcement Learning](https://arxiv.org/abs/2505.08837)
*Muhammad Saqib,Dipkumar Mehta,Fnu Yashu,Shubham Malhotra*

Main category: cs.CR

TL;DR: 论文提出了一种基于强化学习（RL）的动态安全策略管理框架，用于解决云环境中静态安全策略的不足。


<details>
  <summary>Details</summary>
Motivation: 云环境（如AWS）的安全需求复杂且动态变化，静态策略无法应对威胁演化和资源弹性。

Method: 采用深度强化学习算法（如深度Q网络和近端策略优化），利用云遥测数据动态调整防火墙规则和IAM策略。

Result: 实验结果显示，该框架显著优于静态策略，入侵检测率提高至92%（静态为82%），事件检测和响应时间减少58%。
...
### [221] [Evaluating the Robustness of Adversarial Defenses in Malware Detection Systems](https://arxiv.org/abs/2505.09342)
*Mostafa Jafari,Alireza Shameli-Sendi*

Main category: cs.CR

TL;DR: 论文提出了两种方法：优先二进制舍入技术和sigma-binary攻击，用于在二进制特征空间中实现高效对抗攻击，并揭示了现有防御系统的脆弱性。


<details>
  <summary>Details</summary>
Motivation: 现有的机器学习恶意软件检测系统容易受到对抗攻击，且缺乏全面的评估框架来理解其鲁棒性。

Method: 1. 优先二进制舍入技术，将连续扰动转换为二进制特征空间；2. sigma-binary攻击，一种针对二进制域的新型对抗方法。

Result: sigma-binary攻击在Malscan数据集上表现优异，攻击成功率高达94.56%，显著优于现有防御方法。

Conclusion: 研究强调了精确方法（如sigma-binary）的重要性，以暴露现有防御的隐藏漏洞，并支持开发更鲁棒的恶意软件检测系统。
...
### [222] [Toward Malicious Clients Detection in Federated Learning](https://arxiv.org/abs/2505.09110)
*Zhihao Dou,Jiaqi Wang,Wei Sun,Zhuqing Liu,Minghong Fang*

Main category: cs.CR

TL;DR: SafeFL是一种新算法，旨在准确识别联邦学习中的恶意客户端，通过生成合成数据集区分恶意与良性模型，表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 联邦学习的去中心化特性使其易受投毒攻击，现有防御方法效果有限或误判率高，需更精准的检测技术。
...
### [223] [Detecting Sybil Addresses in Blockchain Airdrops: A Subgraph-based Feature Propagation and Fusion Approach](https://arxiv.org/abs/2505.09313)
...
<details>
  <summary>Details</summary>
Motivation: Sybil攻击对区块链生态系统构成严重威胁，尤其是在代币空投事件中，亟需高效识别方法。

Method: 构建双层交易子图，提取时间、金额和网络结构特征，结合lightGBM进行分类。

Result: 在包含193,701个地址的数据集上，所有指标（精度、召回率、F1分数和AUC）均超过0.9。

Conclusion: 该方法可扩展至其他区块链安全领域，如交易操纵识别和流动性风险评估，助力构建更安全的区块链生态系统。
...
### [224] [Evaluating GPT- and Reasoning-based Large Language Models on Physics Olympiad Problems: Surpassing Human Performance and Implications for Educational Assessment](https://arxiv.org/abs/2505.09438)
*Paul Tschisgale,Holger Maus,Fabian Kieser,Ben Kroehs,Stefan Petersen,Peter Wulff*

Main category: physics.ed-ph

TL;DR: 研究比较了通用大语言模型（GPT-4o）和推理优化模型（o1-preview）与德国物理奥林匹克竞赛参与者在解决物理问题上的表现，发现LLMs平均表现优于人类，并探讨了其对物理教育评估的影响。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs在教育中的普及，担心其可能绕过学习过程并影响评估完整性，因此研究其在物理问题解决中的能力，以指导其合理应用。

Method: 通过比较GPT-4o（使用不同提示技术）和o1-preview与德国物理奥林匹克竞赛参与者在解决特定问题上的表现，分析LLMs的优劣势。

Result: GPT-4o和o1-preview在解决物理问题上表现优异，平均优于人类参与者；o1-preview表现更稳定且优于GPT-4o。

Conclusion: 研究为物理教育中如何设计评估提供了建议，包括维护评估完整性和引导学生批判性使用LLMs。
...
TL;DR: 本文提出了一种名为ARCANE的框架，用于实时检测太阳风数据中的行星际日冕物质抛射（ICMEs），通过比较机器学习方法和阈值基线方法，验证了ResUNet++模型的优越性。


<details>
  <summary>Details</summary>
Motivation: ICMEs是空间天气扰动的主要驱动因素，对技术基础设施和人类活动构成风险，因此需要实时检测以实现早期预警。

Method: 提出了ARCANE框架，结合ResUNet++模型和阈值基线方法，在流式太阳风数据中进行ICMEs检测。

Result: ResUNet++模型显著优于基线方法，尤其是在高影响事件检测中，使用实时数据仅导致轻微性能下降，F1得分为0.53。

Conclusion: ARCANE框架为自动化空间天气监测迈出了重要一步，为实时预报能力奠定了基础。
...
### [226] [Independent Component Analysis by Robust Distance Correlation](https://arxiv.org/abs/2505.09425)
*Sarah Leyder,Jakob Raymaekers,Peter J. Rousseeuw,Tom Van Deuren,Tim Verdonck*

Main category: stat.CO

TL;DR: 提出了一种鲁棒的独立成分分析方法RICA，通过最小化距离相关性（dCor）来估计独立成分，并引入bowl变换增强鲁棒性。
...
### [227] [EDBench: Large-Scale Electron Density Data for Molecular Modeling](https://arxiv.org/abs/2505.09262)
*Hongxin Xiang,Ke Li,Mingquan Liu,Zhixiang Cheng,Bin Yao,Wenjie Du,Jun Xia,Li Zeng,Xin Jin,Xiangxiang Zeng*

Main category: physics.chem-ph

TL;DR: 论文提出了EDBench，一个大规模、高质量的电子密度数据集，旨在推动电子尺度的学习研究，并展示了基于学习的方法在计算电子密度上的高效性和准确性。


<details>
  <summary>Details</summary>
Motivation: 现有分子机器学习力场（MLFFs）通常忽略电子密度（ED）的重要性，而ED对准确理解分子力场至关重要。由于ED计算依赖耗时的一阶原理密度泛函理论（DFT），缺乏大规模ED数据限制了其在MLFFs中的应用。

Method: 基于PCQM4Mv2构建EDBench数据集，提供330万分子的精确ED数据，并设计了一套以ED为中心的基准任务（预测、检索和生成）来评估模型能力。

Result: 评估表明，从EDBench学习不仅可行，还能达到高精度。基于学习的方法能高效计算ED，且计算成本显著低于传统DFT。

Conclusion: EDBench为电子密度驱动的药物发现和材料科学奠定了坚实基础，所有数据和基准将免费开放。
...
### [228] [Design of a Formation Control System to Assist Human Operators in Flying a Swarm of Robotic Blimps](https://arxiv.org/abs/2505.09511)
*Tianfu Wu,Jiaqi Fu,Wugang Meng,Sungjin Cho,Huanzhe Zhan,Fumin Zhang*

Main category: cs.RO

TL;DR: 提出了一种基于动态领导者切换机制的室内飞艇群编队控制系统，通过分布式领导者角色增强编队稳定性。


<details>
  <summary>Details</summary>
Motivation: 飞艇群编队控制在复杂环境中实现协调行为的需求，尤其是室内动态环境下的稳定性问题。

Method: 采用领导者-跟随者策略，结合动态领导者切换机制，仅手动控制领导者飞艇，跟随者飞艇通过单目相机和激光高度计估计相对位置和高度。

Result: 实验证明，动态领导者切换机制能有效维持编队稳定性，适应动态环境并辅助人工操作。

Conclusion: 动态领导者切换机制显著提升了飞艇群编队的稳定性和适应性，尤其在复杂机动中表现优异。

Abstract: Formation control is essential for swarm robotics, enabling coordinated
...
### [229] [RT-cache: Efficient Robot Trajectory Retrieval System](https://arxiv.org/abs/2505.09040)
...
<details>
  <summary>Details</summary>
Motivation: 现代视觉-语言-动作模型在机器人任务中推理成本高，导致延迟问题。RT-cache旨在通过大数据检索和学习经验解决这一问题。

Method: RT-cache结合Memory Builder和Trajectory Retrieval，存储大规模成功轨迹并检索相关片段，减少推理开销。

Result: 实验表明，RT-cache在Open-X Embodiment Dataset等数据上比无检索的基线更快、更成功地完成任务。
...
### [230] [Air-Ground Collaboration for Language-Specified Missions in Unknown Environments](https://arxiv.org/abs/2505.09108)
...
TL;DR: 论文提出了一种基于自然语言的无人机与地面机器人协作系统，利用大型语言模型（LLM）实现语义推理和动态任务规划。


<details>
  <summary>Details</summary>
Motivation: 随着自主机器人系统的成熟，用户希望通过自然语言而非低层指令指定任务，但实现这一目标需克服语义推理、异构机器人协调和间歇通信等技术挑战。

Method: 系统采用LLM驱动的规划器，结合在线构建的语义-度量地图，实现无人机与地面机器人的协作任务执行，支持动态任务调整。

Result: 在城乡环境中，系统成功完成了七种不同自然语言任务，导航范围达公里级。

Conclusion: 该系统首次实现了基于自然语言的无人机与地面机器人协作，展示了语义推理和动态规划的潜力。
...
### [231] [ChicGrasp: Imitation-Learning based Customized Dual-Jaw Gripper Control for Delicate, Irregular Bio-products Manipulation](https://arxiv.org/abs/2505.08986)
...
TL;DR: ChicGrasp是一个硬件-软件协同设计的系统，用于自动化抓取和提升家禽尸体，解决了传统方法的不可靠性问题。


<details>
  <summary>Details</summary>
Motivation: 传统家禽加工线依赖人工操作，而现有自动化方法因家尸体的易变形性和解剖学差异难以可靠工作。

Method: 采用双爪气动夹持器和条件扩散策略控制器，通过50次多视角演示训练，实现5自由度末端执行器运动规划。

Result: 系统抓取成功率为40.6%，完成一次抓取到悬挂的周期为38秒，优于现有基准方法。
...
### [232] [Parameter-Efficient Fine-Tuning of Vision Foundation Model for Forest Floor Segmentation from UAV Imagery](https://arxiv.org/abs/2505.08932)
*Mohammad Wasil,Ahmad Drak,Brennan Penfold,Ludovico Scarton,Maximilian Johenneken,Alexander Asteroth,Sebastian Houben*

Main category: cs.RO

TL;DR: 论文提出了一种基于Segment Anything Model (SAM)的方法，通过参数高效微调（PEFT）来分割森林地面对象，解决了森林监测中的挑战。


<details>
  <summary>Details</summary>
Motivation: 森林地面对象的详细理解因高自然变异性、快速变化的环境参数和模糊标注而具有挑战性。

Method: 采用参数高效微调（PEFT）方法，调整SAM的掩码解码器以自动分割森林地面对象。

Result: 基于适配器的PEFT方法实现了最高的平均交并比（mIoU），而低秩适应（LoRA）则为资源受限的无人机平台提供了轻量级替代方案。

Conclusion: 该方法有效解决了森林地面对象分割问题，为无人机在森林监测中的应用提供了技术支持。
...
### [233] [Multi-step manipulation task and motion planning guided by video demonstration](https://arxiv.org/abs/2505.08949)
...
TL;DR: 该论文提出了一种基于教学视频的机器人任务与运动规划方法，扩展了RRT规划器，结合视频中的接触状态和3D物体位姿，解决了具有顺序依赖性的复杂任务。


<details>
  <summary>Details</summary>
Motivation: 利用教学视频指导机器人完成复杂的多步骤任务与运动规划任务，解决传统规划算法在顺序依赖性任务中的局限性。

Method: 扩展RRT规划器，结合视频提取的接触状态和3D物体位姿，设计轨迹优化方法（OCP）实现真实机器人任务执行。

Result: 在多个机器人平台上验证了方法的有效性，包括Franka Emika Panda和KUKA KMR iiwa，并设计了新的基准任务。

Conclusion: 视频引导的规划方法能够有效解决复杂任务，并具有超越视频场景的泛化能力。
...
### [234] [FoldNet: Learning Generalizable Closed-Loop Policy for Garment Folding via Keypoint-Driven Asset and Demonstration Synthesis](https://arxiv.org/abs/2505.09109)
*Yuxing Chen,Bowen Xiao,He Wang*

Main category: cs.RO

TL;DR: 论文提出了一种合成服装数据集方法，用于机器人服装折叠任务，通过关键点生成几何模板和纹理，结合闭环模仿学习训练策略，并引入KG-DAgger提升鲁棒性，最终在真实世界中达到75%的成功率。


<details>
  <summary>Details</summary>
Motivation: 由于服装的可变形性，为机器人服装操作任务生成大量高质量数据具有挑战性。

Method: 构建基于关键点的几何服装模板，生成真实纹理；利用关键点注释生成模拟折叠演示，通过闭环模仿学习训练策略；提出KG-DAgger方法生成恢复失败的演示数据。

Result: KG-DAgger显著提升模型性能，真实世界成功率提高25%；训练15K轨迹后，真实世界成功率达75%。

Conclusion: 实验验证了所提框架在模拟和真实环境中的有效性。
...
### [235] [TransDiffuser: End-to-end Trajectory Generation with Decorrelated Multi-modal Representation for Autonomous Driving](https://arxiv.org/abs/2505.09315)
...
Method: 提出TransDiffuser，一种基于编码器-解码器的生成轨迹规划模型，利用多模态条件输入和表示解相关优化机制。

Result: 在NAVSIM基准测试中达到94.85的PDMS，超越现有方法。

Conclusion: TransDiffuser无需基于锚点的先验轨迹，即可生成高质量多样化的轨迹，性能优越。
...
### [236] [Imitation Learning for Adaptive Control of a Virtual Soft Exoglove](https://arxiv.org/abs/2505.09099)
...
<details>
  <summary>Details</summary>
Motivation: 现有可穿戴机器人在康复训练中常忽视患者肌肉损失的独特性。

Method: 结合强化学习和生物力学模型，通过仿真训练定制化控制器，利用视频数据学习人类抓取任务，并模拟肌肉力量减弱以补偿。

Result: 虚拟可穿戴机器人手套提供了共享辅助，使操纵器在肌肉力量减弱的情况下仍能保持90.5%的原始操作熟练度。

Conclusion: 该方法有效补偿了特定肌肉缺陷，为个性化康复训练提供了新思路。
...
### [237] [APR-Transformer: Initial Pose Estimation for Localization in Complex Environments through Absolute Pose Regression](https://arxiv.org/abs/2505.09356)
*Srinivas Ravuri,Yuan Xu,Martin Ludwig Zehetner,Ketan Motlag,Sahin Albayrak*

Main category: cs.RO

TL;DR: APR-Transformer利用图像或LiDAR数据预测绝对位姿，在GNSS受限环境中表现优异，并在多个数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 精确初始化对定位算法性能至关重要，尤其在GNSS受限环境中。现有深度学习方法在复杂空间关系估计中表现优越，但仍有改进空间。

Method: 提出APR-Transformer模型，基于图像或LiDAR数据预测3D位置和方向，并在多个数据集上测试。

Result: 在Radar Oxford Robot-Car、DeepLoc和自定义APR-BeIntelli数据集上达到SOTA性能，并在实时自动驾驶测试中验证可靠性。

Conclusion: APR-Transformer在GNSS受限环境中具有实际可行性和高效性，代码已开源。
...
### [238] [Deploying Foundation Model-Enabled Air and Ground Robots in the Field: Challenges and Opportunities](https://arxiv.org/abs/2505.09477)
*Zachary Ravichandran,Fernando Cladera,Jason Hughes,Varun Murali,M. Ani Hsieh,George J. Pappas,Camillo J. Taylor,Vijay Kumar*

Main category: cs.RO

TL;DR: 论文探讨了如何将基础模型（FMs）应用于机器人在大规模非结构化环境中的实际部署，提出了SPINE框架，并展示了首个基于大型语言模型（LLM）的机器人规划系统在复杂环境中的成功应用。


<details>
  <summary>Details</summary>
Motivation: 现有基于FMs的机器人主要在封闭环境中运行，而实际任务需要机器人在非结构化环境中自主探索和操作。本文旨在解决这一挑战。

Method: 提出了SPINE框架，支持机器人在非结构化环境中进行大规模任务规划，并通过模型蒸馏技术实现小型语言模型在资源受限平台上的运行。

Result: 展示了首个基于LLM的机器人在复杂环境中完成数公里任务的实例，并成功开发了首个基于设备端语言模型的无人机规划器。

Conclusion: 论文提出了未来研究的多个方向，强调了在非结构化环境中部署FM-enabled机器人的潜力。
...
### [239] [Learning Long-Context Diffusion Policies via Past-Token Prediction](https://arxiv.org/abs/2505.09561)
*Marcel Torne,Andy Tang,Yuejiang Liu,Chelsea Finn*

Main category: cs.RO

TL;DR: 提出了一种名为Past-Token Prediction（PTP）的辅助任务，通过预测过去和未来的动作令牌来改善长上下文策略的性能，并引入多阶段训练策略和自验证机制，显著提升了策略效果和训练效率。


<details>
  <summary>Details</summary>
Motivation: 在机器人任务中，长序列观察和动作的推理至关重要，但现有方法因内存需求和虚假相关性导致性能下降，通常通过截断上下文长度来回避问题，丢弃了关键历史信息。

Method: 提出PTP辅助任务，通过预测过去动作令牌来正则化策略；采用多阶段训练策略，先预训练视觉编码器，再微调策略头；引入自验证机制在测试时选择一致性候选动作。

Result: 在四个现实任务和六个模拟任务中，方法将长上下文扩散策略性能提升3倍，训练速度加快10倍以上。
...
### [240] [Train a Multi-Task Diffusion Policy on RLBench-18 in One Day with One GPU](https://arxiv.org/abs/2505.09430)
*Yutong Hu,Pinhao Song,Kehan Wen,Renaud Detry*

Main category: cs.RO

TL;DR: Mini-Diffuser通过Level-2 minibatching和架构改进，显著减少多任务视觉语言机器人扩散策略的训练时间和内存使用，性能接近SOTA。


<details>
  <summary>Details</summary>
Motivation: 传统扩散策略在训练时间和内存使用上效率低下，而机器人动作的维度远低于图像生成目标，这一不对称性未被充分利用。

Method: 引入Level-2 minibatching，将多个噪声动作样本与每个视觉语言条件配对，并改进扩散变换器架构以防止信息泄漏。

Result: 在RLBench模拟中，Mini-Diffuser达到SOTA性能的95%，仅需5%的训练时间和7%的内存。真实实验验证其保留扩散策略优势。

Conclusion: Mini-Diffuser高效且性能接近SOTA，适用于多模态动作建模和多样化感知输入条件的行为生成。
...
### [241] [Distilling Realizable Students from Unrealizable Teachers](https://arxiv.org/abs/2505.09546)
*Yujin Kim,Nathaniel Chin,Arnav Vasudev,Sanjiban Choudhury*

Main category: cs.RO

TL;DR: 研究在特权信息下的策略蒸馏，学生策略仅能通过部分观察学习全状态访问的教师策略，解决信息不对称问题。


<details>
  <summary>Details</summary>
Motivation: 信息不对称导致学生无法直接访问教师的状态空间，引发分布偏移和策略退化。现有方法效率低下。

Method: 提出两种方法：(i) 自适应决定何时查询教师的模仿学习方法，(ii) 选择初始化训练位置的强化学习方法。

Result: 在模拟和真实机器人任务中验证，显著提升了训练效率和最终性能。

Conclusion: 通过策略性交互和高效探索，解决了信息不对称问题，优于基线方法。
...
### [242] [DataMIL: Selecting Data for Robot Imitation Learning with Datamodels](https://arxiv.org/abs/2505.09603)
*Shivin Dass,Alaa Khaddaj,Logan Engstrom,Aleksander Madry,Andrew Ilyas,Roberto Martín-Martín*

Main category: cs.RO

TL;DR: DataMIL是一种基于策略的数据选择框架，通过端到端方式优化数据选择，提升机器人策略在特定任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的大规模数据集训练的策略在多样化任务上表现良好，但在特定任务上表现不佳，需要进一步调整。

Method: 引入DataMIL框架，利用策略自身选择能提升性能的数据点，避免基于人工质量标准的低效选择。

Result: 在60多个模拟和真实世界任务中验证，DataMIL显著提升了成功率，优于多个基线方法。

Conclusion: 端到端的性能感知数据选择是释放大型数据集潜力的关键。
...
### [243] [Template-Guided Reconstruction of Pulmonary Segments with Neural Implicit Functions](https://arxiv.org/abs/2505.08919)
*Kangxian Xie,Yufei Zhu,Kaiming Kuang,Li Zhang,Hongwei Bran Li,Mingchen Gao,Jiancheng Yang*

Main category: cs.GR

TL;DR: 提出了一种基于神经隐式函数的方法，用于高质量3D重建肺段，解决了传统深度学习方法在计算资源和分辨率上的限制。


<details>
  <summary>Details</summary>
Motivation: 肺段的高质量3D重建对肺段切除术和肺癌手术规划至关重要，但现有方法在计算资源和分辨率上存在不足。

Method: 采用神经隐式函数学习3D表面，通过变形可学习模板实现解剖感知的精确重建，并引入两个临床相关评估指标。

Result: 提出的方法优于现有方法，并开发了一个名为Lung3D的公开数据集，包含800个标记肺段的3D模型。
...
### [244] [IntrinsicEdit: Precise generative image manipulation in intrinsic space](https://arxiv.org/abs/2505.08889)
...
TL;DR: 提出了一种基于RGB-X扩散框架的通用生成工作流，支持像素级精确的语义和局部编辑，无需额外数据或微调。
...
Method: 在固有图像潜在空间中操作，结合精确扩散反演和分离通道处理，解决身份保持和通道纠缠问题。

Result: 在复杂图像的多种编辑任务（如颜色调整、对象插入/移除、全局光照调整）中表现优异。

Conclusion: 该方法实现了高效、精确的多任务编辑，解决了现有方法的局限性。
...
### [245] [Neural BRDF Importance Sampling by Reparameterization](https://arxiv.org/abs/2505.08998)
*Liwen Wu,Sai Bi,Zexiang Xu,Hao Tan,Kai Zhang,Fujun Luan,Haolin Lu,Ravi Ramamoorthi*

Main category: cs.GR

TL;DR: 提出一种基于重参数化的神经BRDF重要性采样方法，提升渲染效率与灵活性。
...
Result: 在神经BRDF渲染中实现最佳方差减少，同时保持高推理速度。

Conclusion: 该方法为神经BRDF重要性采样提供了更高效、灵活的解决方案。
...
### [246] [UMotion: Uncertainty-driven Human Motion Estimation from Inertial and Ultra-wideband Units](https://arxiv.org/abs/2505.09393)
*Huakun Liu,Hiroki Ota,Xin Wei,Yutaro Hirao,Monica Perusquia-Hernandez,Hideaki Uchiyama,Kiyoshi Kiyokawa*

Main category: cs.GR

TL;DR: UMotion是一个基于不确定性驱动的框架，结合IMU和UWB传感器，通过UKF实时优化3D人体姿态和形状估计，解决了姿态模糊和数据漂移问题。
...
Method: 提出UMotion框架，结合6个UWB距离传感器和IMU，通过UKF实时融合传感器数据与人体运动约束。

Result: 实验表明UMotion能稳定传感器数据，并在姿态精度上优于现有技术。

Conclusion: UMotion通过不确定性驱动的融合方法，显著提升了3D人体姿态和形状估计的准确性和鲁棒性。
